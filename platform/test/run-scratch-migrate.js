...
// run-scratch-migrate.js — behavioral coverage for the scratch day-migration
// helpers. Autoloop queue item cov-blueprint-scratch-customjs-behavioral (5/8):
// the 3 uncovered methods were ScratchDayMigrate.migrate (real frontmatter-repair
// logic), ScratchDayMigrateInit.invoke (startup orchestration wrapper), and a
// widget render() (already covered by run-scratch-render-guards.js #212). This
// harness covers the two genuinely-behavioral ones:
//
//  * ScratchDayMigrate.migrate(file) — recovers a broken/missing `day` frontmatter
...
//  * ScratchDayMigrateInit.invoke() — must not throw (its try/catch swallows).
//
...
const ScratchDayMigrate = load('platform/blueprints/scratch/helpers/scratch-day-migrate.js', 'ScratchDayMigrate');
const ScratchDayMigrateInit = load('platform/blueprints/scratch/helpers/scratch-day-migrate-init.js', 'ScratchDayMigrateInit');
...
(async () => {
  const mig = new ScratchDayMigrate();

  // Case 1: already a valid quoted YYYY-MM-DD → no-op.
  {
    const fm = { day: '2026-07-02' };
    installApp(fm);
    const changed = await mig.migrate({ path: 'spice/scratch/2026-07-02/note.md' });
  ...
  // Case 3: non-string (Date) day, date recoverable from FILENAME.
  {
    ...
    installApp(fm);
    const changed = await mig.migrate({ path: 'spice/scratch/2026-07-02.md' });
  ...
  // Case 3b: filename with suffix (YYYY-MM-DD-morning.md).
  {
    const fm = { day: 12345 };
    installApp(fm);
    const changed = await mig.migrate({ path: 'spice/scratch/2026-07-02-morning.md' });
  ...
  // Case 4: missing day, path has NO recoverable date → no-op.
  {
    const fm = {};
    installApp(fm);
    const changed = await mig.migrate({ path: 'spice/scratch/random-note.md' });
  ...
  ok('SM-7 _migrateFrontmatter(null) → false', mig._migrateFrontmatter(null, { path: 'spice/scratch/2026-07-02.md' }) === false);
  ok('SM-8 _extractDateFromPath(non-string) → null', mig._extractDateFromPath(42) === null);
  ok('SM-9 _extractDateFromPath(no-date path) → null', mig._extractDateFromPath('spice/scratch/foo.md') === null);
  ...
  // registers the resync command + runs startup migration via customJS.migrateAll.
  {
    ...
    global.customJS = { ScratchDayMigrate: { migrateAll: async () => { migrateAllCalled = true; return { migrated: 0, scanned: 0, skipped: true }; } } };
    global.app = {
      commands: { addCommand() {} },
    ...
    let threw = false;
    try { await new ScratchDayMigrateInit().invoke(); } catch (_e) { threw = true; }
  ...
  {
    // Dataview ready but customJS.ScratchDayMigrate absent → "unavailable; skipping"
