...
// transform (reused _healChromeBarMigration) against a REALISTIC deployed
// Meeting Hub note body; (2) the file-walking heal against a fake adapter.
...
// ---- pure transform: strips SpaceNavButtons, inserts MeetingChromeBar, leaves EntityCreate/cards content intact ----
{
  const after = _healChromeBarMigration(LEGACY_HUB_BODY, 'meeting', 'MeetingChromeBar');
  ...
  ok('MHCBH-3 leaves the entity-create block (creation still works)', after.includes('entity-create:meeting') && after.includes('class: "EntityCreate"'));
  ok('MHCBH-4 leaves the cards list (MeetingsHubCards) intact', after.includes('class: "MeetingsHubCards"'));
  ok('MHCBH-5 idempotent — already-migrated body is returned unchanged', _healChromeBarMigration(after, 'meeting', 'MeetingChromeBar') === after);
}
...
\`\`\`
`;
  ok('MHCBH-6 a meeting LEAF note (type: meeting, no meetings-hub tag) is not a hub-tag match',
...
async function runAdapterCases() {
  ...
  ok('MHCBH-7 heals the one meetings-hub-tagged note found by the adapter walk',
    out && /class:\s*"MeetingChromeBar"/.test(out) && !out.includes('SpaceNavButtons'));
  const backedUp = Object.keys(adapter.backups).some((p) => p.includes('Meetings-2026-07-01.md'));
  ok('MHCBH-8 snapshots the pre-heal body to .sauce-backup before writing', backedUp);
  ok('MHCBH-9 pushes an info history event naming the healed target',
  ...
  ok('MHCBH-10 idempotent — a second run against an already-healed vault writes nothing',
  ...
  ok('MHCBH-11 skips a file under spice/meetings/hubs that lacks the meetings-hub tag',
