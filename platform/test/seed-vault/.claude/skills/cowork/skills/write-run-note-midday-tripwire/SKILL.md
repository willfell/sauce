...
3. Construct full markdown via existing frontmatter + body assembly (frontmatter from input.frontmatter merged with cadence-canonical fields including `severity`; body from `input.body`).
...
5. Invoke `writeAtomicNote({ mdPath, sidecarPath, body_md, sidecar_json: input.sidecar_json, schemaPath: <vault>/.claude/skills/cowork/data/schemas/midday-tripwire@1.0.0.json })` from `write-atomic-note-helper.js`.
