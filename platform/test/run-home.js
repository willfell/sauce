...
(async () => {
  ...
  // ── HOME-SETTLE: cold-load index-settle gate (_awaitIndexResolved + render) ──
  {
    // _awaitIndexResolved fires on the metadataCache "resolved" event.
    let onCb = null;
    const appEvt = { metadataCache: { resolvedLinks: {}, on: (ev, cb) => { if (ev === "resolved") onCb = cb; return {}; }, offref: () => {} } };
    const p1 = SpaceHome._awaitIndexResolved(appEvt, 5000);
    let ev1 = false; p1.then(() => { ev1 = true; });
    setTimeout(() => { if (onCb) onCb(); }, 5);
    await p1;
    assertTrue("HOME-SETTLE-1 resolves on metadataCache 'resolved' event", ev1);

    // Fail-safe: event never fires → resolves via maxMs timeout.
    const appNever = { metadataCache: { resolvedLinks: {}, on: () => ({}), offref: () => {} } };
    const t0 = Date.now();
    await SpaceHome._awaitIndexResolved(appNever, 25);
    assertTrue("HOME-SETTLE-2 resolves via maxMs timeout when event never fires", Date.now() - t0 >= 18);

    // Already-resolved fast path: resolvedLinks populated → resolve without subscribing.
    let subscribed = false;
    const appWarm = { metadataCache: { resolvedLinks: { "a.md": {} }, on: () => { subscribed = true; return {}; }, offref: () => {} } };
    await SpaceHome._awaitIndexResolved(appWarm, 5000);
    assertTrue("HOME-SETTLE-3 already-resolved fast-path skips subscribing", subscribed === false);
  }

  // Render defers the dashboard mount until the index resolves (first gated render).
  {
    installMoment("2026-07-02", 6);
    const dv = makeDv();
    global.customJS = { SpaceDailyDashboard: { computeCounts: () => ({ today: 0, overdue: 0, done: 0, meetings: 0 }) } };
    global.window.customJS = global.customJS;
    let onCb = null;
    global.app = { metadataCache: { resolvedLinks: {}, on: (ev, cb) => { if (ev === "resolved") onCb = cb; return {}; }, offref: () => {} } };
    global.window.app = global.app;
    global.window.__sauceHomeSettledOnce = false;
    global.window.__sauceHomeLayoutReady = true;
    const rp = home_.render(dv, {});
    await new Promise((r) => setTimeout(r, 8));
    const early = dv._viewCalls.filter((c) => c.input && c.input.class === "SpaceDailyDashboard").length;
    assertEq("HOME-SETTLE-4 dashboard deferred until index resolves", early, 0);
    if (onCb) onCb();
    await rp;
    const after = dv._viewCalls.filter((c) => c.input && c.input.class === "SpaceDailyDashboard").length;
    assertEq("HOME-SETTLE-5 dashboard mounts once the index resolves", after, 1);
    assertTrue("HOME-SETTLE-6 __sauceHomeSettledOnce set after first gated render", global.window.__sauceHomeSettledOnce === true);
    delete global.app; delete global.window.app;
    global.window.__sauceHomeSettledOnce = false;
    global.window.__sauceHomeRenderToken = 0;
  }

  // A superseded render (token bumped mid-settle) bails without mounting.
  {
    installMoment("2026-07-02", 6);
    const dv = makeDv();
    global.customJS = { SpaceDailyDashboard: { computeCounts: () => ({ today: 0, overdue: 0, done: 0, meetings: 0 }) } };
    ...
    const rp = home_.render(dv, {});
    await new Promise((r) => setTimeout(r, 8));
    // A newer render begins while we await the index → bump the token.
    global.window.__sauceHomeRenderToken = (global.window.__sauceHomeRenderToken || 0) + 1;
    if (onCb) onCb();
    await rp;
    const mounted = dv._viewCalls.filter((c) => c.input && c.input.class === "SpaceDailyDashboard").length;
    assertEq("HOME-SETTLE-7 superseded render bails without mounting the dashboard", mounted, 0);
    delete global.app; delete global.window.app;
    delete global.customJS; if (global.window) global.window.customJS = undefined;
    global.window.__sauceHomeSettledOnce = false;
    global.window.__sauceHomeRenderToken = 0;
  }
