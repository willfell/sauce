#!/usr/bin/env node
/**
 * run-mayo-plugin-surface — preflight harness for the loop plugin's Claude
...
const PLUGIN = path.join(REPO, 'plugins', 'mayo');
...
const EXPECTED_SKILLS = ['block-review', 'execute', 'init', 'intake', 'plan', 'propose', 'review', 'run', 'status'];
...
// LP-1: marketplace manifest at repo root points at the plugin subdirectory.
{
  ...
  const entry = (mkt.plugins || []).find((p) => p.name === 'mayo');
  ok('LP-1 loop plugin entry', !!entry);
  ok('LP-1 relative source', entry && entry.source === './plugins/mayo');
...
// LP-2: plugin manifest.
{
  ...
  ok('LP-2 plugin name', manifest.name === 'mayo');
...
// LP-3: per-skill body gates.
{
  ...
  for (const dir of dirs) {
    ...
    ok(`LP-3 ${dir} slash surface documented`, new RegExp(`mayo:${dir}`).test(body) || dir === 'intake' || dir === 'run', 'body should self-name its slash command');
  }
}

// LP-4: portability gate — nothing under plugins/mayo/ may carry machine paths.
{
  const offenders = [];
...
// posture themselves so the start prompt can be identical for every repo.
{
  ...
  ok('LP-7 run documents the universal prompt', /Use \$mayo-run --live\. Start NOW/.test(run));
