...
async function caseHCV0891Versions() {
  ...
  assertEqual(coworkMan.version, "0.39.0", "HC-V0891-VERSION-A: cowork pin = 0.31.0 (v0.93.0 bump)");
  ...
  assertEqual(wsVer, "0.100.1", "HC-V0891-VERSION-C: workshop pin = 0.93.3 (v0.93.3 bump)");
  const pkg = JSON.parse(fs.readFileSync(
    path.resolve(WORKSHOP, "..", "package.json"), "utf8"));
  assertEqual(pkg.version, "0.100.1", "HC-V0891-VERSION-C: package.json = 0.93.3 (v0.93.3 bump)");
...
(async function main() {
...

  {
    ...
    try {
      ...
      assertTrue("HC-V0900-VERSION-A: package.json version === '0.93.3'", pkg.version === "0.100.1");
      ...
      assertTrue("HC-V0900-VERSION-D: ranch platform-subscription cowork pin === '0.31.0'",
        cworkPin && cworkPin.version === "0.39.0");
    } catch (e) {
...
`;
  ...
  // ============================================================================
  // v0.91.1 HC-V0911-* — write-guard + claude_md_row + verbal-commitment
  // ============================================================================

  // A: write-guard in 5 write-run-note sub-skills
  ["morning-briefing", "midday-tripwire", "eod-review", "weekly-review", "monthly-review"].forEach((orchName, idx) => {
    const letter = String.fromCharCode(65 + idx);
    const p = path.join(WORKSHOP, "platform/blueprints/cowork/skills/skills/write-run-note-" + orchName + "/SKILL.md");
    console.log(`\n--- Case HC-V0911-WRITE-GUARD-${letter}1: write-run-note-${orchName} ---`);
    try {
      const body = fs.readFileSync(p, "utf8");
      assertTrue(`HC-V0911-WRITE-GUARD-${letter}1: write-guard v0.91.1 section present`,
        body.includes("v0.91.1 write-guard"));
      assertTrue(`HC-V0911-WRITE-GUARD-${letter}1: REJECTS spice/daily/ paths`,
        body.includes("spice/daily/") && body.includes("REJECT"));
      assertTrue(`HC-V0911-WRITE-GUARD-${letter}1: returns failed:contract-violation:wrong-output-path`,
        body.includes("failed:contract-violation:wrong-output-path"));
    } catch (e) {
      assertTrue(`HC-V0911-WRITE-GUARD-${letter}1: write-guard contract`, false, e && e.message);
    }
  });

  // B: claude_md_row disambiguation in cowork manifest
  {
    console.log("\n--- Case HC-V0911-DISAMBIG-A1: cowork manifest Cowork Atomic Notes row ---");
    try {
      const m = JSON.parse(fs.readFileSync(
        path.join(WORKSHOP, "platform/blueprints/cowork/manifest.json"), "utf8"));
      const row = (m.claude_surface || []).find(c =>
        c.kind === "claude_md_row" && c.row && c.row.topic === "Cowork Atomic Notes (cron output)");
      assertTrue("HC-V0911-DISAMBIG-A1: Cowork Atomic Notes resolver row present", !!row);
      assertTrue("HC-V0911-DISAMBIG-A1: row points at spice/cowork/daily path",
        row && row.row && row.row.path && row.row.path.includes("daily/"));
      assertTrue("HC-V0911-DISAMBIG-A1: row command cites NOT spice/daily disambiguation",
        row && row.row && row.row.command && row.row.command.includes("NOT spice/daily/"));
    } catch (e) {
      assertTrue("HC-V0911-DISAMBIG-A1: disambiguation row contract", false, e && e.message);
    }
  }
  ...
  try {
    ...
    assertTrue("HC-V0920-VERSION-A2: blueprints[].cowork.version === 0.31.2",
      platMan.blueprints.find(b => b.name === "cowork").version === "0.39.0");
    const pkg = JSON.parse(fs.readFileSync(path.join(WORKSHOP, "package.json"), "utf8"));
    ...
    assertTrue("HC-V0920-VERSION-A4: workshop subscription cowork pin === 0.31.0",
      workshopSub.blueprints.find(b => b.name === "cowork").version === "0.39.0");
  } catch (e) {
  ...
  try {
    ...
    assertTrue("HC-V0930-CONTRACT-A2: contract_version === \"0.33.0\"",
      _sjContract.contract_version === "0.34.1");
  ...
  try {
    ...
    assertTrue("HC-V0951-K3-G: scheduled-job-contract.json contract_version bumped to 0.33.0 (was 0.32.0 pre-S3.2)",
      contract.contract_version === "0.34.1");
  } catch (e) {
  ...
  // v0.96.2 — daily-dashboard tsKeys hotfix (HC-V0962-DASH-1..3)
  // ─────────────────────────────────────────────────────────────
  // SpaceDailyDashboard must honor the semantic `day:` frontmatter field when
  // bucketing cowork atomic notes into the Activity panel. Closes the
  // timestamp-drift class where an EOD cron firing past midnight stamped
  // created_at on the wrong calendar day (diagnosed by Workflow wzr3g3uad).
  //
  // DASH-1 — tsKeys arg includes `"day"` as the FIRST element (source-level)
  // DASH-2 — inWindow surfaces a page on day 2026-06-08 when day matches but
  //          created_at is the next morning (4am Tuesday)
  // DASH-3 — inWindow does NOT double-surface that same page on day 2026-06-09
  //          (day takes precedence over created_at)

  const DASHBOARD_SRC = path.join(WORKSHOP, "platform/blueprints/daily/helpers/space-daily-dashboard.js");
  ...
  try {
    ...
    assertTrue("HC-V0970-T-9-contract-version: scheduled-job-contract.json contract_version bumped to 0.35.0 for Rail T migration (pointer-field replaces inline wrapper_template — schema change → MINOR bump)",
      ...
      `expected contract_version "0.35.1", got ${JSON.stringify(contract.contract_version)}`);
  } catch (e) {
