...
3. Read the canonical daily template at `ranch/templates/Daily Note.md` via `mcp__obsidian__read_note`. This template is materialized by the `daily` blueprint and ships the `[//]: # (COWORK_CALLOUTS)` anchor (daily@0.2.5+; HTML-comment → Obsidian `%% %%` → markdown link-ref `[//]: # (...)` in daily@0.8.2 / v0.67.2 for true invisibility in both Reading mode and Live Preview).
