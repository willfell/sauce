...
// run-meetings-render-guards.js — cold-load render coverage for the meetings
// blueprint's render widgets. Autoloop queue item cov-blueprint-meetings-widget-render.
//
// MeetingsHubCards is exercised via run-renderer.js, but MeetingLeafActions (the
// New Task / Add to Project / Edit Attendees action row) had no render()-execution
// test — run-meeting-leaf-actions.js exercises its data-source/onClick helpers, not
// render(). This harness drives BOTH widgets' render() through the cold-load path —
// Dataview not indexed so dv.pages() is empty; MeetingLeafActions renders its three
// AccentButton actions (onClick not invoked during render), MeetingsHubCards hands
// the empty set to BeaconCards — in a normal container AND a `.markdown-embed`
// context (MeetingLeafActions carries the embed early-return guard), asserting no
// throw. Same render-safe net the to-do / cowork / project / scratch / task-entity
// / wiki / trips / teams / people widgets have.
//
...
(async () => {
    for (const w of widgets) {
        ...
        catch (e) { await guard(`MTGGUARD-load ${w.name}`, () => { throw e; }); continue; }
        for (const v of variants) {
            await guard(`MTGGUARD ${w.name} — ${v.label}`, async () => {
