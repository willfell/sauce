...
// run-backlink-panel-render-guards.js — cold-load render coverage for the
// backlink-panel mechanism's render widget. Autoloop queue item
// cov-mechanism-backlink-panel-widget-render.
//
// run-backlink-panel.js unit-tests BacklinkPanel's query/format helpers, but
// render() itself had no execution test. render(dv, {entityType}) requires a valid
// entityType (else it Notices + returns); with one, _reverseQuery guards the
// cold-load path (`dv.current()?.file` -> [] when Dataview is un-indexed) so
// render falls to _renderEmpty. This harness drives render() through that path in
// a normal container AND a `.markdown-embed` context, asserting no throw. Same
// render-safe net the to-do / cowork / project / ... / meetings widgets have.
//
// Stubs mirror run-meetings-render-guards.js (empty dv.pages chainable, tolerant
// DOM proxy with firstChild->null + createEl, chainable window.moment, no-op
// BeaconCards/chrome classes, + a no-op Notice). Zero-dep. "N passed, M failed" —
// exit 0 iff M === 0.
...
// A no-op Notice so any obsidian `new Notice(...)` inside render is inert in Node.
...
const cjs = {
    ...
    // BacklinkPanel delegates its (non-empty) list to BeaconCards; unreached on cold-load.
...
// BacklinkPanel.render(dv, opts) needs a valid entityType; on cold-load its
// _reverseQuery returns [] (dv.current()?.file guard) -> _renderEmpty.
const widgets = [
    { name: 'BacklinkPanel', path: 'platform/mechanisms/backlink-panel/backlink-panel.js', opts: { entityType: 'project' } },
];
...
(async () => {
    for (const w of widgets) {
        ...
        catch (e) { await guard(`BLPGUARD-load ${w.name}`, () => { throw e; }); continue; }
        for (const v of variants) {
            await guard(`BLPGUARD ${w.name} — ${v.label}`, async () => {
                const inst = new WidgetClass();
                await Promise.resolve(inst.render(makeDv(v.embed, v.current), w.opts || {}));
            });
        }
    }
