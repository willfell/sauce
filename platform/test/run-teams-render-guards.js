...
// run-teams-render-guards.js — cold-load render coverage for the teams blueprint's
// render widgets. Autoloop queue item cov-blueprint-teams-widget-render.
//
// TeamsHubCards / TeamPageCards / TeamActionButtons had no render()-execution
// test. Each opens with a `.markdown-embed` early-return guard; on a normal
// container with Dataview un-indexed, TeamsHubCards queries the empty
// dv.pages('"spice/teams"') and renders its "No teams yet" empty state,
// TeamPageCards renders its rollup-pending stub, and TeamActionButtons renders the
// "+ New Team" AccentButton (its onClick is not invoked during render). This
// harness drives each widget's render() through those paths in a normal container
// AND a `.markdown-embed` context, asserting no throw. Same render-safe net the
// to-do / cowork / project / scratch / task-entity / wiki / trips widgets have.
//
// Stubs mirror run-trips-render-guards.js (empty dv.pages chainable, tolerant DOM
// proxy with firstChild→null + createEl, chainable window.moment, no-op
// BeaconCards/AccentButton/chrome classes). Zero-dep. "N passed, M failed" — exit
// 0 iff M === 0.
...
const cjs = {
    ...
    // TeamsHubCards delegates its (non-empty) grid to BeaconCards; unreached on cold-load.
...
const widgets = [
    { name: 'TeamsHubCards',     path: 'platform/blueprints/teams/scripts/teams-hub-cards.js' },
    { name: 'TeamPageCards',     path: 'platform/blueprints/teams/scripts/team-page-cards.js' },
    { name: 'TeamActionButtons', path: 'platform/blueprints/teams/scripts/team-action-buttons.js' },
];

// cold-load variants: Dataview not indexed → dv.current() undefined/null and
// dv.pages() empty; plus the embed context (all three carry the .markdown-embed
// early-return guard).
const variants = [
...
(async () => {
    for (const w of widgets) {
        ...
        catch (e) { await guard(`TEAMGUARD-load ${w.name}`, () => { throw e; }); continue; }
        for (const v of variants) {
            await guard(`TEAMGUARD ${w.name} — ${v.label}`, async () => {
