...
async function dispatch(argv, opts) {
    ...
    if (verb === "doctor") {
        const cmd = require(VERBS.doctor);
        // Context-free: read-only platform health check. Forward test hooks
        // (_brewPrefix, _nodeVersion) via the opts arg to dispatch().
        const testCtx = opts || {};
        await cmd.run(testCtx, rest);
        return;
    }
    ...
    if (verb === "unlink") {
        const cmd = require(VERBS.unlink);
        await cmd.run(null, rest);
        return;
    }
    if (!VERBS[verb]) {
        throw new Error(`unknown verb: ${verb}\nUsage: sauce <bootstrap|update|status|wizard|migrate|migrate-layout|audit|vault|reinstall|doctor|link|unlink|help>`);
    }
    let ctx;
    if (verb === "bootstrap") {
