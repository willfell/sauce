...
// v0.60.0 — quote an arbitrary string as a POSIX-sh single-quoted literal.
// Single-quoted bash strings do not undergo $VAR / backtick / \ expansion;
// only ' itself needs escaping via the standard '\'' close-reopen idiom.
// Used by phaseWriteActivation when interpolating filesystem paths into
// the generated activate.sh + sauce shim. Exported (underscore prefix) so
// run-helper-cases.js can round-trip-verify it through real bash.
...
SAUCE_VAULT=${_shellSingleQuote(vaultPath)}
export SAUCE_VAULT
_SAUCE_SCRIPTS_DIR=${_shellSingleQuote(scriptsDir)}
case ":$PATH:" in
  *":$_SAUCE_SCRIPTS_DIR:"*) ;;
  *) export PATH="$_SAUCE_SCRIPTS_DIR:$PATH" ;;
esac
unset _SAUCE_SCRIPTS_DIR
