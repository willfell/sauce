...
7. Build the callouts list:
   ...
   - If `yellow_charges.length > 0`: Use Skill `cowork:write-callout-tripwire-yellow` with `{ date: context.today, charges: <yellow_charges rendered as markdown table>, scheels_today_total: "$<step 4.scheels_total>", mtd_discretionary: "Discretionary month-to-date: $<step 4.mtd_discretionary>.", days_since_last_splurge: "<step 4.days_since_splurge_pre> days clean before this charge." }`. Capture `yellow_md`.
