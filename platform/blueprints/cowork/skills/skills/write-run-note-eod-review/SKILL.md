...
Before composing the body, set frontmatter `title` to: `<Display Name> — <weekday>, <month_name> <D>, <year>` where `<D>` is the integer day (no leading zero) and `<year>` is the 4-digit year from `date`. For this skill the display-name literal is **`EOD Review`**. Example for Friday 2026-05-22: `"EOD Review — Friday, May 22, 2026"`.
...
- Optional `> [!example]+ 🧩 Notes thematically close to today` callout, placed AFTER the cadence's primary example block(s) and BEFORE the closing `> [!tip]`. Renders only when gather-semantic-related returned status == "ready". The body-shape pre-write self-check already accepts multiple example markers; no contract change needed.
...
```yaml
type: cowork-eod-review             # MUST be this EXACT string
...
1. `type:` is missing OR != the EXACT string `cowork-eod-review`.
2. Any of `engagement_id`, `day`, `generator`, `prompt_source`, `title`, `summary` is missing/empty.
3. Frontmatter contains non-canonical fields like `cadence:`, `date:`, `generated_at:`.
4. `day:` not in ISO `YYYY-MM-DD` format.
...
**Frontmatter checks:**
- [ ] `type:` matches the canonical value for this skill (`cowork-eod-review`)
