...
class JournalChromeBar {
  ...
  _setVisibleBannerTitle(dv, value) {
    try {
      ...
      const banners = container.querySelectorAll(".journal-title-banner") || [];
  ...
  async _writeTitle(dv, file, current, next, onDone) {
    ...
    if (!renderSafe || typeof renderSafe.mutate !== "function") {
      if (typeof Notice === "function") new Notice("JournalChromeBar: RenderSafe is unavailable.", 6000);
      return false;
    }
    const result = await renderSafe.mutate({
      ...
      dv,
      path: file.path,
      failureMessage: "Could not rename journal entry",
