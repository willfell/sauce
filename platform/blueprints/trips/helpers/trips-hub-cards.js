class TripsHubCards {
  async render(dv) {
    ...
    const tripDate = (p) => parseDate(p.start_date) || parseDate(p.date);
    ...
    await window.customJS.BeaconCards.render(dv, {
      ...
      meta: (p) => {
        ...
        const d = tripDate(p);
        const isPast = d && d.isBefore(today);
      ...
      sort: (a, b) => {
        const da = tripDate(a);
        const db = tripDate(b);
        const aUp = da && da.isSameOrAfter(today);
        const bUp = db && db.isSameOrAfter(today);
        if (aUp && bUp) return da.valueOf() - db.valueOf();
        if (aUp && !bUp) return -1;
        if (!aUp && bUp) return 1;
        if (!da) return 1;
        if (!db) return -1;
