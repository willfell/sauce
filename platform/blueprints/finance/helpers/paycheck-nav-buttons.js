 ...
 * `beacon/finance/paychecks/<YYYY-MM-DD>/`. Renders buttons for Paycheck /
 * Paychecks Hub / Finance Hub. Hides the button matching the active file.
 * Embed-deduped per v0.16.0 lesson. Mirrors InvoiceNavButtons shape.
 */
class PaycheckNavButtons {
    async render(dv) {
        ...
        const targets = [
            { label: "Paycheck",      target: `beacon/finance/paychecks/${start_date}/Paycheck-${start_date}.md` },
            { label: "Paychecks Hub", target: `beacon/finance/paychecks/Paychecks.md` },
            { label: "Finance Hub",   target: `beacon/finance/Finance.md` }
        ].filter(t => t.target !== path);

        const row = dv.container.createEl("div", { cls: "pnb-root" });
