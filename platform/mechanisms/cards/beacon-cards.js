 ...
 *              "outline" renders a colored dot + alpha-blend bg + 999px pill (Linear/Github aesthetic).
...
class BeaconCards {
    ...
    _renderBadges(card, page, ctx) {
        ...
        const outlinePalette = {
            accent:  { dot: "var(--interactive-accent)", border: "var(--interactive-accent)", bg: "transparent" },
            warn:    { dot: "#f59e0b", border: "rgba(245, 158, 11, 0.35)", bg: "rgba(245, 158, 11, 0.08)" },
            error:   { dot: "#dc2626", border: "rgba(220, 38, 38, 0.35)",  bg: "rgba(220, 38, 38, 0.08)" },
        ...
        for (const b of badges) {
            ...
            const style = b.style === "outline" ? "outline" : "fill";
            const chip = badgeRow.createEl("span");
            if (style === "outline") {
                const p = outlinePalette[tone] || outlinePalette.muted;
                chip.style.cssText = `display: inline-flex; align-items: center; gap: 6px; padding: 1px 10px 1px 8px; border-radius: 999px; font-size: 0.72em; font-weight: 500; background: ${p.bg}; color: var(--text-normal); border: 1px solid ${p.border}; white-space: nowrap; letter-spacing: 0.02em;`;
                ...
                const p = fillPalette[tone] || fillPalette.muted;
