...
vars:
  ...
  pr_command: cd "${vars.worktree}" && git push -u origin "${vars.branch}" && gh pr create --fill >/dev/null && gh pr view --json number,url
nodes:
  - id: claim
    type: shell
  ...
  - id: implement
    ...
    prompt: |
      ...
      Never touch versions, tags, release PRs, or the Homebrew tap.
  - id: adequacy
    type: judge
    run: ${vars.adequacy_command}
  - id: lens-correctness
    type: agent
    worker: claude-code
    isolate: none
  ...
  - id: fix-ci
    ...
    prompt: |
      ...
      regression test honest, and commit. Outcome pass when CI should be green.
  - id: refresh
    type: agent
    worker: claude-code
    cwd: ${vars.worktree}
    prompt: |
      ...
      the tests, and push. Outcome pass when the branch is current.
  - id: supersede
    type: human
...
edges:
  ...
  - { from: adequacy, to: implement, on: fail, max: 1 }
  - { from: adequacy, to: supersede, on: exhausted }
  - { from: lens-correctness, to: record-correctness, on: always }
  - { from: record-correctness, to: lens-regression, on: pass }
  - { from: record-correctness, to: implement, on: refute, max: 1 }
  - { from: record-correctness, to: supersede, on: exhausted }
  - { from: lens-regression, to: record-regression, on: always }
  - { from: record-regression, to: lens-adequacy, on: pass }
  - { from: record-regression, to: implement, on: refute, max: 1 }
  - { from: record-regression, to: supersede, on: exhausted }
  - { from: lens-adequacy, to: record-adequacy, on: always }
  - { from: record-adequacy, to: verify-gates, on: pass }
  - { from: record-adequacy, to: implement, on: refute, max: 1 }
  - { from: record-adequacy, to: supersede, on: exhausted }
  - { from: verify-gates, to: pr, on: gates-passed }
  - { from: verify-gates, to: implement, on: fail, max: 1 }
  - { from: verify-gates, to: supersede, on: exhausted }
  ...
  - { from: advance, to: implement, on: needs-implementation, max: 1 }
