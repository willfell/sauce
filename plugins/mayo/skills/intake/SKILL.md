...
1. Resolve: `node "${CLAUDE_PLUGIN_ROOT}/scripts/loop-config.js" resolve --json` from the repo root; refusal → `/mayo:init`, stop. If `config.policy.observe_only` is true, refuse: this binding is bind-and-observe.
