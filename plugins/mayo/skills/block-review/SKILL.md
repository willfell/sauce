...
1. Resolve: `node "${CLAUDE_PLUGIN_ROOT}/scripts/loop-config.js" resolve --json`; refusal → `/mayo:init`, stop. Refuse if `config.policy.observe_only` unless the Director explicitly authorizes a supervised lift for this pass.
...
For each `needs_decision` item, ask ONE question, recommendation first: **mint** the missing foundation via `/mayo:intake`, or **confirm-clear** (the dep is obsolete/folded elsewhere → run `node <coordinator> reconcile-dependencies --card "<dependent>" --clear "<dead dep>" --reason "<director confirmation>" --apply --json`). Never mint or clear a judgment item without the Director's word.
