...
1. Resolve: `node "${CLAUDE_PLUGIN_ROOT}/scripts/loop-config.js" resolve --json` from the repo root; refusal → `/mayo:init`, stop.
...
2. Walk the `since` section item by item, newest first:
   - **Self-ratified FID amendments** (heading + date): one line each. The Director may accept silently (default) or **reject** — rejection spawns a corrective card through `/mayo:intake`; shipped work stands per its receipts. Never revert an amendment in place.
...
3. Capture the answer. Policy changes go through the FID as amendments; work changes go through `/mayo:intake`. Never edit a card, the board, or coordinator state.
