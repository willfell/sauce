...
- **Cycle order:** v0.1.0 ✅ → v0.1.1 ✅ → v0.1.x ✅ → v0.1.3 ✅ → v0.1.2 ✅ → v0.2.0 ✅ → **next TBD (v0.2.1 project migration to `beacon/projects/` OR v0.3.0 next-blueprint)**.
- **Workshop version:** `0.6.0` (bumped from 0.5.0 in v0.2.0 S2 — boards blueprint + module_directory invariant + new schema fields; tag `v0.6.0` annotated and pushed).
- **Mechanisms:** `customjs-guard@1.0.0`, `validator@0.1.1`, `audit@0.1.1`, `nav-buttons@2.0.0` (no mechanism version bumps in v0.2.0).
- **Blueprints:** `project@0.3.0` (v0.2.0 BREAKING: nav_buttons[] empty — Board contribution moved to boards; module_directory: "projects-legacy" stopgap; content/kanban-board.md removed from files[]), `boards@0.1.0` NEW (Kanban-plugin board + Templater date-routed cards under `beacon/boards/`; declares external_plugins[obsidian-kanban] + pre_install[delete legacy boards/To-Do-Board.md]).
- **Two load-bearing harnesses:** `platform/test/run-install.js` + `platform/test/run-renderer.js`. v0.1.3 added `platform/test/run-helper-cases.js`; v0.2.0 grew it from 16 → 119 sub-asserts (14 new cases: M1, M2, M3, M4, M5, O1, O2, O3, O4, O5, P1, P2, P3, P4 covering the four new installer mechanics).
- **New manifest schema fields (v0.2.0 additive):** `module_directory: "<bare-name>"` (REQUIRED on every blueprint; mechanisms exempt) — installer derives `beacon/<bare-name>` for the per-blueprint `{{module_directory}}` substitution; `pre_install[]` (optional) — list of pre-files-loop actions; one action type currently supported: `{ type: "delete", path: "<dest-relative>", reason: "<why>" }`.
- **Landmines list:** 14 entries (no new landmines in v0.2.0; #11 module-directory invariant is now codified in the installer per v0.2.0 S1).
