...
- **Cycle order:** v0.1.0 ✅ → v0.1.1 ✅ → v0.1.x ✅ → v0.1.3 ✅ → v0.1.2 ✅ → v0.2.0 ✅ → v0.3.0 ✅ → **next TBD (v0.2.1 project migration to `beacon/projects/` OR v0.3.x meetings/to-do siblings OR v0.4.0 invoke_command nav-button action type for an optional Today button)**.
- **Workshop version:** `0.7.0` (bumped from 0.6.0 in v0.3.0 S2 — daily blueprint + `core_plugin_settings[]` schema + module_directory mkdir mechanic; tag `v0.7.0` annotated and pushed).
- **Mechanisms:** `customjs-guard@1.0.0`, `validator@0.1.1`, `audit@0.1.1`, `nav-buttons@2.0.0` (no mechanism version bumps in v0.3.0).
- **Blueprints:** `boards@0.1.0` (Kanban board + date-routed cards under `beacon/boards/`), `daily@0.1.0` NEW (date-organized daily notes under `beacon/daily/` via core Daily Notes plugin + ported `SpaceDailyDashboard`; declares `core_plugin_settings[daily-notes]`), `project@0.3.0` (v0.2.0 BREAKING: Board contribution moved to boards; `module_directory: "projects-legacy"` stopgap).
- **Two load-bearing harnesses:** `platform/test/run-install.js` + `platform/test/run-renderer.js`. v0.1.3 added `platform/test/run-helper-cases.js`; v0.2.0 grew it from 16 → 119 sub-asserts (14 new cases). v0.3.0 grew it from 119 → 172 sub-asserts (5 new cases C1-C5 for `applyCorePluginSettings` + 1 case M6 for module_directory mkdir; +53 sub-asserts).
- **New manifest schema fields (v0.3.0 additive):** `core_plugin_settings[]` (optional) — list of `{ id, settings }` entries; installer additive-shallow-merges into `.obsidian/<id>.json` (subject to landmine #12's four safety mechanics). Settings string values pass through `substituteLenient` using the per-blueprint variables overlay (nested objects replaced wholesale; substitution variables must be JSON-safe scalars per helper header).
- **New installer mechanics (v0.3.0):** (1) `applyCorePluginSettings` helper wired in `installItem` after `applySlashCommanderBindings`; (2) module_directory mkdir at `installItem` between `applyPreInstall` and the `files[]` loop (T-FIX1; codifies landmine #11 directly in the installer).
- **Landmines list:** 14 entries (no new landmines in v0.3.0; #11 codification tightened by T-FIX1; #12 allowlist text updated to 3 paths).
