...
- **Workshop version:** `0.5.0` (bumped from 0.4.1 in v0.1.2 S2 — distribution-model shift; tag `v0.5.0` annotated and pushed; tag `v0.4.1` retro-applied to commit `25ef276`).
- **Mechanisms:** `customjs-guard@1.0.0`, `validator@0.1.1` (v0.1.3: declares `templater_hotkeys[Validate.md]` + `slash_commander_bindings[/validate]`), `audit@0.1.1` (v0.1.3: declares `templater_hotkeys[Audit.md]` + `slash_commander_bindings[/audit]`), `nav-buttons@2.0.0`.
- **Blueprints:** `project@0.2.1` (v0.1.3: declares `templater_hotkeys[Create New Project.md]` + `slash_commander_bindings[/new-project]`; workshop subscription DROPPED project — barebones-only dogfood). v0.2.0 will bump to `project@0.3.0` (Board retired) + add `boards@0.1.0`.
- **Two load-bearing harnesses:** `platform/test/run-install.js` + `platform/test/run-renderer.js`. v0.1.3 added `platform/test/run-helper-cases.js` (16 sub-asserts, 4 cases × 2 helpers, tmpdir scratch vault per case).
- **Landmines list:** 14 entries (#13 stub content-static, #14 `gitState()` best-effort — codified in v0.1.2 S2 alongside the distribution-model shift).
