...
4c. composeBody emits the feedback callout per `{{shared.feedback_capture_template}}`
    ...
    its parser remains in the reconciler for the historical corpus. (Weekly is
    eligible — only anti-echo is excluded at week-tier.)
...
7a. **Compute the EXACT output path** (no improvisation, no daily-note pattern):
    ...
    Example for 2026-06-10 weekly-review: `spice/cowork/daily/2026/06-June/2026-06-10/weekly-review.md`.
    DO NOT write to `spice/cowork/daily/2026/06-June/Tuesday-2026-06-10.md` or any other shape.

7b. **Emit the frontmatter as a literal YAML block** (this exact shape, no extra keys, no missing keys; for weekly, `day:` is the ISO week-anchor day and there is NO `week:` key in this writer contract):
...
`Outside yesterday's frame` (weekly-review skips anti-echo by default — gate
