...
3a. **Prep synopsis_md.** Compose `> [!info]+ Where the month landed` callout per
    `prompt_body` AND the § Synopsis composition rules below (voice-shaped one-paragraph
    synopsis distilled from gather outputs; ≤80 words; first-sentence-concrete; empty-day
    "Quiet day" fallback). When `semantic_index_age` is non-null, append
...
4c. composeBody emits the feedback callout per `{{shared.feedback_capture_template}}`
    ...
    its parser remains in the reconciler for the historical corpus. (Monthly is
    eligible — only anti-echo is excluded at month-tier.)
...
`Outside yesterday's frame` (monthly-review skips anti-echo by default — gate
closed unless the engagement opts in via render_aspects). The coverage-gap
injection (expected vs surfaced kinds) is non-failing — visible warning
callout + sidecar `coverage_gap` field so reconcile-cowork can monitor
cross-day.
