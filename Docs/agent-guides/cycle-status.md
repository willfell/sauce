...
- **Workshop version:** `0.78.0` (closed 2026-05-27)
- **Most recent cycle:** v0.78.0 cowork-consumption MINOR — closes the consumption gap that v0.76.0 + v0.77.0 left open: the 5 cowork atomic-note orchestrators (morning-briefing / midday-tripwire / eod-review / weekly-review / monthly-review) now READ `spice/cowork/context/user-preferences.md` at fire-time and shape their output accordingly. Two new vendor-agnostic sub-skills do the heavy lifting: `cowork:read-user-preferences` (pure parse + `{ prefs, status }` code) and `cowork:gather-from-served-by` (one gather path for BOTH `override_classified` known kinds (M365 UUID gateway serves calendar+email+chat) AND user-defined `custom_kind` kinds (ado / github / Backstage / NewRelic with free-text `what_matters`)). Each orchestrator gains pre-flight step 3b (read prefs), step 3c (plan dispatch from `prefs.priorities[]`), step 13b (voice-contract prefix composed from `prefs.personality` — vibe / formality / pep_talk / length / notes), and an updated step 14 (compose-body-helper end-to-end). Gather phase becomes a priority-ordered loop instead of a hardcoded sequence. In-position `[!warning]` callouts replace example blocks for kinds that are unclassified, disconnected at capture time, or whose `served_by` namespace is unreachable at fire time. Legacy fallback bit-for-bit preserved when prefs are empty / missing / malformed: every orchestrator falls through to the v0.77.0 hardcoded gather sequence unchanged. Forcing function: the accuris vault's 2026-05-27 morning-briefing.md ignored captured priorities, personality, and the three classified custom kinds entirely. Cowork 0.16.0 → 0.17.0.

## Cycle order (chronological)

v0.1.0 → v0.1.1 → v0.1.x → v0.1.3 → v0.1.2 → v0.2.0 → v0.3.0 → v0.4.0 → v0.3.2 → v0.4.2 → v0.5.0 → v0.11.0 → v0.12.0 → v0.13.0 → v0.14.0 → v0.6.0 → v0.16.0 → v0.17.0 → v0.18.0/.1/.2 → v0.19.0 → v0.20.0 → v0.21.0/.1 → v0.22.0/.1 → v0.23.0 → v0.24.0 → v0.25.0 → v0.26.0/.1 → v0.27.0 → v0.28.0 → v0.29.0 → v0.30.0 ⏭️ → v0.31.0 → v0.32.0 → v0.33.0/.1 → v0.36.0/.1 → v0.37.0 → v0.38.0/.1 → v0.40.0 → v0.41.0/.5 → v0.42.0 → v0.43.0 → v0.44.0 → v0.45.0 → v0.46.0/.1/.2 → v0.47.0 → v0.48.0 → v0.49.0 → v0.49.1 ⏭️ → v0.49.2 → (v0.50.0–v0.62.0 narratives lost; pre-v0.63 narrative below resumes) → v0.63.0 → v0.63.1 → v0.63.2 → v0.63.3 → v0.64.0 → v0.64.1 → v0.64.2 → v0.64.3 → v0.65.0 → v0.66.0 → v0.66.1 → v0.66.2 → v0.67.0 → v0.67.1 → v0.67.2 → v0.67.3 → v0.70.0 → v0.70.1 → v0.70.2 → v0.70.3 → v0.70.4 → v0.70.5 → v0.70.6 → v0.70.7 → v0.71.0 → v0.71.1 → v0.72.0 → v0.72.1 → v0.73.0 → v0.74.0 → v0.75.0 → v0.75.1 → v0.76.0 → v0.77.0 → v0.78.0 (current).
...
- **v0.79.0 candidate: personality hard rules** — v0.78.0's voice-contract prefix is 1–4 sentences of soft guidance. Add a `personality.hard_rules[]` list whose entries are appended verbatim (do-not-interpret) to the prefix (e.g., `never use the word 'leverage'`).
