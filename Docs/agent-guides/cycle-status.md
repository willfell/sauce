...
- **Workshop version:** `0.78.1` (closed 2026-05-29)
- **Most recent cycle:** v0.78.1 cowork-consumption-fix PATCH — SKILL.md prose-only patch closing two v0.78.0 bugs exposed by the accuris 2026-05-28 morning-briefing autonomous run. (A) Agent-side algorithm primary, helper harness-only: `read-user-preferences/SKILL.md` and the 5 orchestrator step 3c blocks no longer tell the agent to delegate to the v0.78.0 helper at `.local/blueprints/cowork/helpers/<name>-helper.js` (which never exists in consumer vaults — the helpers are workshop-only for harness use). The agent-side algorithm is now the primary documented behavior; a new `## Harness testing` section explicitly notes the helper is NOT materialized in consumer vaults. (B) Source URL contract in `gather-from-served-by`: new `## Source URL requirements` section with MUST/SHOULD per-kind discipline (github + ado + email = MUST; calendar + chat + finance = SHOULD; custom kinds = SHOULD), per-kind URL-shape examples, and a fallback `_did not expose URL fields_` note pattern for genuinely-no-URL runs. 7 SKILL.md files modified. +2 HC-V0781-* static prose-lint cases (A1 + B1) in `run-cowork-smoke.js`. No new helpers, no manifest schema change. Cowork blueprint stays at 0.17.0 (prose-only patch).

## Cycle order (chronological)

v0.1.0 → v0.1.1 → v0.1.x → v0.1.3 → v0.1.2 → v0.2.0 → v0.3.0 → v0.4.0 → v0.3.2 → v0.4.2 → v0.5.0 → v0.11.0 → v0.12.0 → v0.13.0 → v0.14.0 → v0.6.0 → v0.16.0 → v0.17.0 → v0.18.0/.1/.2 → v0.19.0 → v0.20.0 → v0.21.0/.1 → v0.22.0/.1 → v0.23.0 → v0.24.0 → v0.25.0 → v0.26.0/.1 → v0.27.0 → v0.28.0 → v0.29.0 → v0.30.0 ⏭️ → v0.31.0 → v0.32.0 → v0.33.0/.1 → v0.36.0/.1 → v0.37.0 → v0.38.0/.1 → v0.40.0 → v0.41.0/.5 → v0.42.0 → v0.43.0 → v0.44.0 → v0.45.0 → v0.46.0/.1/.2 → v0.47.0 → v0.48.0 → v0.49.0 → v0.49.1 ⏭️ → v0.49.2 → (v0.50.0–v0.62.0 narratives lost; pre-v0.63 narrative below resumes) → v0.63.0 → v0.63.1 → v0.63.2 → v0.63.3 → v0.64.0 → v0.64.1 → v0.64.2 → v0.64.3 → v0.65.0 → v0.66.0 → v0.66.1 → v0.66.2 → v0.67.0 → v0.67.1 → v0.67.2 → v0.67.3 → v0.70.0 → v0.70.1 → v0.70.2 → v0.70.3 → v0.70.4 → v0.70.5 → v0.70.6 → v0.70.7 → v0.71.0 → v0.71.1 → v0.72.0 → v0.72.1 → v0.73.0 → v0.74.0 → v0.75.0 → v0.75.1 → v0.76.0 → v0.77.0 → v0.78.0 → v0.78.1 (current).
