...
- **Workshop version:** `0.84.4` (closed 2026-06-02)
- **Most recent cycle:** v0.84.4 polish-pills PATCH — three CSS polish tweaks on v0.84.3's right-aligned pills. §1 adjacent-sibling rule suppresses the chevron's `margin-left: auto` when counts precedes it (was competing with the counts auto-margin and putting pills in the middle of the row). §2 inter-pill gap 6px → 10px. §3 count-pill border `--background-modifier-border` → `-hover` (more findable). Two lockstep bumps (workshop / daily 0.13.4). 3 new HC-V0844-A* sub-asserts. Single cycle commit. Preflight ALL GREEN.
