...
- **Workshop version:** `0.76.0` (closed 2026-05-23)
- **Most recent cycle:** v0.76.0 cowork-interactive-context MINOR — eight workstreams (A-H) bundling four v0.75.1 carry-forward bug fixes with the first half of cowork's interactive-context personalization feature. (A) `_resolveWorkshopPath` ancestry-walk anchor changed from `process.execPath` to `__filename` — the v0.75.1 helper bug was hidden by the test mock; real brew installs never resolved the workshop. (B) Stale `installed.workshop_path` values are invalidated via a `platform/manifest.json` existence probe (handles `brew cleanup` between sauce upgrades). (C) eod-review + weekly-review get the v0.75.1 H morning-briefing fix — semantic-warning gated on gather step having run + canonical text from gather-semantic-related's contract. (D) eod/weekly/monthly orchestrators get explicit engagement-template prompt fallback when user prompt is empty. (E) Regression coverage for v0.59.9's `materialize_once` flag (discovered during writing-plans; reused with no install.js code change). (F) New `cowork:context-builder` orchestrator-tier skill — 14 hand-curated questions across 4 MCP-kinds (calendar / gmail / imessage / finance) + priorities + personality; writes `spice/cowork/context/user-preferences.md` (protected by `materialize_once: true`). (G) New `Docs/agent-guides/cowork-customization-contract.md` documenting STOCK / USER / USER-DRAFTABLE-WITH-BACKUP boundary. (H) `cowork:onboard-scheduled-jobs` auto-delegates to context-builder when user-preferences is absent or seed-stamped. Cowork 0.14.1 → 0.15.0.

## Cycle order (chronological)

v0.1.0 → v0.1.1 → v0.1.x → v0.1.3 → v0.1.2 → v0.2.0 → v0.3.0 → v0.4.0 → v0.3.2 → v0.4.2 → v0.5.0 → v0.11.0 → v0.12.0 → v0.13.0 → v0.14.0 → v0.6.0 → v0.16.0 → v0.17.0 → v0.18.0/.1/.2 → v0.19.0 → v0.20.0 → v0.21.0/.1 → v0.22.0/.1 → v0.23.0 → v0.24.0 → v0.25.0 → v0.26.0/.1 → v0.27.0 → v0.28.0 → v0.29.0 → v0.30.0 ⏭️ → v0.31.0 → v0.32.0 → v0.33.0/.1 → v0.36.0/.1 → v0.37.0 → v0.38.0/.1 → v0.40.0 → v0.41.0/.5 → v0.42.0 → v0.43.0 → v0.44.0 → v0.45.0 → v0.46.0/.1/.2 → v0.47.0 → v0.48.0 → v0.49.0 → v0.49.1 ⏭️ → v0.49.2 → (v0.50.0–v0.62.0 narratives lost; pre-v0.63 narrative below resumes) → v0.63.0 → v0.63.1 → v0.63.2 → v0.63.3 → v0.64.0 → v0.64.1 → v0.64.2 → v0.64.3 → v0.65.0 → v0.66.0 → v0.66.1 → v0.66.2 → v0.67.0 → v0.67.1 → v0.67.2 → v0.67.3 → v0.70.0 → v0.70.1 → v0.70.2 → v0.70.3 → v0.70.4 → v0.70.5 → v0.70.6 → v0.70.7 → v0.71.0 → v0.71.1 → v0.72.0 → v0.72.1 → v0.73.0 → v0.74.0 → v0.75.0 → v0.75.1 → v0.76.0 (current).
...
- **v0.77.0 candidate: 5-orchestrator consumption of `user-preferences.md`** — wire morning-briefing / midday-tripwire / eod-review / weekly-review / monthly-review to read user-preferences via a new `cowork:read-user-preferences` sub-skill; apply priority-reordering of `[!example]+` blocks; apply personality voice in narrative sections. This is the v0.77.0 cycle topic — closes the consumption layer that v0.76.0's F workstream stabilized the file schema for.
- **v0.77.0 candidate: promote 5 cowork prompts to strict-USER (`materialize_once`) OR formalize the `.bak` behavior** — currently the 5 prompts are USER-DRAFTABLE-WITH-BACKUP (in `files[]` without `materialize_once`; `.bak`'d-then-overwritten via v0.2.0 Option B). Pick one direction and codify it.
- **v0.77.0 candidate: 5-orchestrator engagement-template fallback parity** — v0.76.0's S6 covered eod/weekly/monthly. Morning-briefing + midday-tripwire still rely on implicit `onboard-scheduled-jobs` install-time seeding; add explicit fire-time fallback.
- **v0.77.0 candidate: sc-bridge fallback anchor when calendar empty** (from v0.75.0/v0.75.1) — bridge stays cold when morning-briefing fires with no calendar events; fire sc-bridge with vault-root anchor to keep model warm.
- **v0.77.0 candidate: `--subscribe <mech>=<ver>` CLI flag** (from v0.75.0/v0.75.1) — new-in-workshop mechanism subscription still requires manual jq edit.
- **v0.77.0 candidate: HC-V0751-A1 registry-driven** (from v0.75.1) — hardcodes 3 engagement-type names; should discover dynamically from `platform/blueprints/cowork/engagement-types/*.json`.
- **v0.77.0 candidate: factor shared test utilities** (from v0.75.1) — `assertEqual`/`assertTrue`/UNIT_TEST_MODE duplicated in run-install.js + run-cli.js; extract to `platform/test/harness-utils.js`.
- **v0.77.0 candidate: check-claude-surface harness** (from v0.75.1) — assert dest content matches what source would produce (prevents install.md source-vs-dest drift).
