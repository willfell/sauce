...
- **FLN-v82-1 (HIGH PRIORITY): engagement-type-accessibility fix.** Engagement-type JSON manifests (`platform/blueprints/cowork/engagement-types/*.json`) are declared in cowork's `engagement_types[]` but NOT in `files[]` — they never materialize to consumer vaults. Surfaced in v0.82.0's S0 investigation of the accuris midday-tripwire silent no-op: the workshop JSON had the right `tripwire_aspects` since v0.13.0, but the orchestrator on a consumer vault has no canonical path to read it. Three candidate designs: (a) add JSONs to cowork's `files[]` so they materialize to `spice/cowork/context/engagement-types/*.json` + update midday-tripwire/SKILL.md to reference materialized path; (b) embed `tripwire_aspects` (+ other manifest-derived runtime fields) per-engagement in vault-config.md + have `cowork:bootstrap-vault` seed defaults from manifest; (c) hardcode aspect mappings per engagement type into the orchestrator SKILL.md. Required for any engagement-type-driven orchestrator behavior to actually reach consumers. v0.83.0 candidate.
- **FLN-v82-2: ranch/rules/cowork.json dedup.** Install appends rather than dedupes rule_fragments; every dogfood install adds 15 path_glob fragments. File has bloated to ~7900 lines with 13 duplicate copies of each path_glob (HEAD-of-v0.81.1 already had 12 copies). Pre-existing across cycles since at least v0.78.0 — every cycle's S9 commits the same ~+630-line delta. Doesn't break preflight (semantic dedup likely happens at rule-evaluation time) but file size will keep growing. Add unique-key dedup at install-time (in `applyRuleFragment` or the rules-merge layer).
- **FLN-v82-3: parseYamlIsh hyphenated-key support.** Pre-existing limitation in `context-builder-dry-run.js#parseYamlIsh` line 35: flat-key regex `[a-z_]+` silently drops keys like `my-custom-kind` from `user-preferences.md`. Surfaced when authoring HC-V0820-A4 (had to switch test kind from `my-custom-kind` to `notion` for parser compatibility). Custom kinds with hyphens (e.g., `lharries-whatsapp`, `sharepoint-online`) can't appear in user-preferences.md today. Widen regex to `[a-z_][a-z0-9_-]*`.
