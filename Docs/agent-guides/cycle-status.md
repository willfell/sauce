...
v0.1.0 → v0.1.1 → v0.1.x → v0.1.3 → v0.1.2 → v0.2.0 → v0.3.0 → v0.4.0 → v0.3.2 → v0.4.2 → v0.5.0 → v0.11.0 → v0.12.0 → v0.13.0 → v0.14.0 → v0.6.0 → v0.16.0 → v0.17.0 → v0.18.0/.1/.2 → v0.19.0 → v0.20.0 → v0.21.0/.1 → v0.22.0/.1 → v0.23.0 → v0.24.0 → v0.25.0 → v0.26.0/.1 → v0.27.0 → v0.28.0 → v0.29.0 → v0.30.0 ⏭️ → v0.31.0 → v0.32.0 → v0.33.0/.1 → v0.36.0/.1 → v0.37.0 → v0.38.0/.1 → v0.40.0 → v0.41.0/.5 → v0.42.0 → v0.43.0 → v0.44.0 → v0.45.0 → v0.46.0/.1/.2 → v0.47.0 → v0.48.0 → v0.49.0 → v0.49.1 ⏭️ → v0.49.2 → (v0.50.0–v0.62.0 narratives lost; pre-v0.63 narrative below resumes) → v0.63.0 → v0.63.1 → v0.63.2 → v0.63.3 → v0.64.0 → v0.64.1 → v0.64.2 → v0.64.3 → v0.65.0 → v0.66.0 → v0.66.1 → v0.66.2 → v0.67.0 → v0.67.1 → v0.67.2 → v0.67.3 → v0.70.0 → v0.70.1 → v0.70.2 → v0.70.3 → v0.70.4 → v0.70.5 → v0.70.6 → v0.70.7 → v0.71.0 → v0.71.1 → v0.72.0 → v0.72.1 → v0.73.0 → v0.74.0 → v0.75.0 → v0.75.1 → v0.76.0 → v0.77.0 → v0.78.0 → v0.78.1 → v0.79.0 → v0.80.0 (current).
...
- **FLN-v80-1: HC-V0800-A1 glob-prose regex over-strict.** The plan's A1 regex required the literal substring `per-mcp/<kind_name>/*.md` in orchestrator step 2c prose; the plan's verbatim prose split the literal across two phrases. Fixed inline at S7 by amending prose to "files matching the `per-mcp/<kind_name>/*.md` glob"; applied identically across all 5 orchestrators in S8. Carry-forward: either soften the regex or codify the literal-glob phrasing in the plan-template HC skeleton.
- **FLN-v80-2: landmine #16 less load-bearing than v0.79.0 close documented.** Per-stage `node platform/test/run-install.js . --auto-approve` did regenerate `.claude/skills/cowork/*` dests at every install in v0.80.0 — the per-item version short-circuit does NOT prevent dest catch-up for surfaces processed by `materializeClaudeSurface`. Future plans can stop predicting "deferred to S11" for SKILL.md dest drift; the global aggregate step keeps them current.
