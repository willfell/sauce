...
| Reader | `ReaderQueue` (`platform/blueprints/reader/helpers/reader-queue.js:42`) | **OK** — queue status re-buckets immediately through `RenderSafe.mutateStructure` (`:424`) before surface-serialized persistence, without mutating Dataview's authority snapshot or forcing a global refresh. | **OK** — render generations keep the newest DocSearch mode authoritative over older receipts; rollback preserves a newer still-connected active control (including the permanent search strip), while absent an intervening render whole-container receipts serialize across articles and restore exact prior child nodes/model/toggle state/order. Successful gestures resolve triggering focus both after optimistic render and again at persistence settlement; rejected gestures do so after rollback, using owned/clicked/active/visible targets or the focusable results container when a row becomes hidden. | **OK** — current page, throwing cold-load access, and dependencies are guarded. | **OK** — one queue query, sorted and re-bucketed in memory. |
