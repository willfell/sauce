...
| Reader | `ReaderQueue` (`platform/blueprints/reader/helpers/reader-queue.js:42`) | **OK** — queue status re-buckets immediately through `RenderSafe.mutateStructure` (`:414`) before serialized persistence, without mutating Dataview's authority snapshot or forcing a global refresh. | **OK** — rejection restores the exact prior child nodes, local status/toggle state, row order, and triggering focus; successful replacement controls receive focus. | **OK** — current page, throwing cold-load access, and dependencies are guarded. | **OK** — one queue query, sorted and re-bucketed in memory. |
