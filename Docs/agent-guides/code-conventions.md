...
| Reader | `ReaderQueue` (`platform/blueprints/reader/helpers/reader-queue.js:42`) | **OK** — queue status re-buckets immediately through `RenderSafe.mutateStructure` (`:418`) before surface-serialized persistence, without mutating Dataview's authority snapshot or forcing a global refresh. | **OK** — whole-container receipts serialize across articles and resolve triggering focus at execution to a still-live owned/clicked/active/visible target (or the focusable results container when a row becomes hidden); rejection restores exact prior child nodes, local status/toggle state, and row order. | **OK** — current page, throwing cold-load access, and dependencies are guarded. | **OK** — one queue query, sorted and re-bucketed in memory. |
