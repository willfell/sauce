...
| Reader | `ReaderQueue` (`platform/blueprints/reader/helpers/reader-queue.js:42`) | **OK** — queue status re-buckets immediately through `RenderSafe.mutateStructure` (`:424`) before surface-serialized persistence, without mutating Dataview's authority snapshot or forcing a global refresh. | **OK** — render generations keep the newest DocSearch mode authoritative over older receipts; rollback preserves a newer still-connected active control (including the permanent search strip), while absent an intervening render whole-container receipts serialize across articles, restore exact prior child nodes/model/toggle state/order, and resolve triggering focus to a still-live owned/clicked/active/visible target (or the focusable results container when a row becomes hidden). | **OK** — current page, throwing cold-load access, and dependencies are guarded. | **OK** — one queue query, sorted and re-bucketed in memory. |
