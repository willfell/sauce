...
| Reader | `ReaderQueue` (`platform/blueprints/reader/helpers/reader-queue.js:42`) | **OK** — queue status re-buckets immediately through `RenderSafe.mutateStructure` (`:418`) before surface-serialized persistence, without mutating Dataview's authority snapshot or forcing a global refresh. | **OK** — whole-container receipts serialize across articles and rebind click-time focus to current live toggle ownership at execution; rejection restores the exact prior child nodes, local status/toggle state, row order, and triggering focus while successful replacement controls receive focus. | **OK** — current page, throwing cold-load access, and dependencies are guarded. | **OK** — one queue query, sorted and re-bucketed in memory. |
