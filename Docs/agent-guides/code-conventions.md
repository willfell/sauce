...
| To-do | `ToDoDailyFilterView` (`todo-daily-filter-view.js:146`) | **OK** — completion gestures delegate to the shared task row; scope, sort, Done, and By Project controls are client-only presentation state. | **OK** — inherited from the shared task row; control repaints retain their owning container. | **OK** — missing current page, TaskEntity, or TaskTodayList dependencies fail closed. | **OK** — one bulk task-note query feeds every scope, sort, and grouping mode. |
...
| Project | `ProjectMeetingsList` (`project-meetings-list.js:203`) | **N/A** — read-only per-project meetings list (month-grouped BeaconCards rows on the project-meetings page). | **N/A** | **OK** — embed guard, guarded RenderSafe page resolve, missing project ref renders nothing, outer never-throw boundary. | **OK** — one scoped meetings query plus one bulk task-count query, never per row. |
...
| Meetings | `MeetingChromeBar` (`platform/blueprints/meetings/helpers/meeting-chrome-bar.js:1`), `MeetingLeafActions` (`meeting-leaf-actions.js:9`), `MeetingsBrowseList` (`meetings-browse-list.js:27`), `MeetingsHubCards` (`meetings-hub-cards.js:14`), `PeopleRendering` (`platform/mechanisms/people-rendering/people-rendering.js:17`), `SectionLabel` | **OK/N/A** — New Meeting and new-person creation use receipt-bound `RenderSafe.mutateStructure` previews; project and attendee edits use `RenderSafe.mutate` optimistic controls without mutating Dataview's authority snapshot; lists remain read-only. | **OK/N/A** — RenderSafe captures scroll first; rejection restores exact person-list node identities and triggering-control state/focus while mutation modals remain mounted with the user's selected data for retry. | **OK** — all four Meetings render entries guard undefined, null, file-less, throwing-current, and missing-dependency states; iterable DataArrays are normalized without assuming native Arrays. | **OK** — the browse and compatibility hubs correlate bounded meeting, task, and people snapshots in memory with zero per-meeting body reads. |
...
| Reader | `ReaderQueue` (`platform/blueprints/reader/helpers/reader-queue.js:42`) | **OK** — queue status re-buckets immediately through `RenderSafe.mutateStructure` (`:424`) before surface-serialized persistence, without mutating Dataview's authority snapshot or forcing a global refresh. | **OK** — render generations keep the newest DocSearch mode authoritative over older receipts; rollback preserves a newer still-connected active control (including the permanent search strip), while absent an intervening render whole-container receipts serialize across articles and restore exact prior child nodes/model/toggle state/order. Successful gestures preserve newer connected focus and resolve triggering focus after queued optimistic apply and again at persistence settlement; rejected gestures do so after rollback, using owned/clicked/active/visible targets or the focusable results container when a row becomes hidden. | **OK** — current page, throwing cold-load access, and dependencies are guarded. | **OK** — one queue query, sorted and re-bucketed in memory. |
| Reader | `ReaderArticleView` (`reader-article-view.js:36`), `ReaderChromeBar` (`reader-chrome-bar.js:14`; status writes delegate to `reader-article-actions.js:189`) | **OK/N/A** — article status has optimistic label rollback; view/chrome are otherwise read-only. | **OK/N/A** — lifecycle captures for status. | **OK** — `ReaderArticleView`, delegated `ReaderArticleActions`, and `ReaderChromeBar` fail closed through undefined, file-only, throwing-current, and missing-dependency cold-load states. | **N/A** — active-note frontmatter. |
...
| Wiki | `WikiTree` (`platform/blueprints/wiki/helpers/wiki-tree.js:1`), `WikiHubActions` (`wiki-hub-actions.js:18`), `WikiLeafActions` (`wiki-leaf-actions.js:14`) | **OK** — create, rename, move, bulk-move, and delete gestures enter the shared receipt-bound `RenderSafe.mutateStructure` lifecycle before filesystem persistence; authoritative reconciliation remains natural and no global refresh is forced. | **OK** — mechanism-owned identities restore the exact tree/card node, title, parent/sibling position, and triggering focus on rejection. | **OK** — folder truth, current-page reads, render entries, and click-time dependencies fail closed through missing or throwing cold-load state. | **OK** — one scoped wiki tree query; move options compute on click. |
| Wiki | `WikiChromeBar` (`wiki-chrome-bar.js:10`) | **OK/inherited** — leaf move and entity creation delegate to the same receipt-bound Wiki/EntityCreate lifecycles; chrome navigation is read-only. | **OK/inherited** | **OK** — RenderSafe page guard and guarded dependencies. | **OK/N/A** — one scoped lookup where context requires it. |
| Home | `SpaceHome` (`platform/blueprints/home/helpers/space-home.js:44`) | **OK** — quick task capture applies an optimistic preview through `RenderSafe.mutateStructure` before persistence and reconciles naturally; the global refresh is retained only for the clock-driven day-rollover watcher, not as a structural shortcut. | **OK** — rejection restores the exact input value, selection, disabled state, menu state, and triggering focus while removing only the owned preview node. | **OK** — cold-start/layout/current-page/dependency guards keep the render path fail-closed. | **OK** — delegates dashboard data rather than issuing per-card queries. |
...
| Daily | `SpaceDailyDashboard` (`platform/blueprints/daily/helpers/space-daily-dashboard.js:113`) | **OK** — To-do creation applies an owned optimistic preview through `RenderSafe.mutateStructure` before persistence; task-row gestures inherit the shared task lifecycle and no structural global refresh is used. | **OK** — rejection removes the exact preview, restores triggering focus, and compensates a newly created empty month folder. | **OK** — missing/throwing Dataview state, current-page reads, and dependencies fail closed through the outer never-throw render boundary. | **GAP PERF-10** — ten `dv.pages` call sites repeat broad scans in one composite render. |
