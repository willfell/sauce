...
| Project | `ProjectChromeBar` (`platform/blueprints/project/helpers/project-chrome-bar.js:35`) | **GAP PERF-2** — archive toggle is a bare frontmatter write and the helper retains a global refresh call. | **GAP PERF-2** — no receipt/focus restoration around archive state. | **OK** — `RenderSafe.page` owns current-page fallback. | **N/A** |
| Project | `ProjectStatusWidget` (`project-status-widget.js:9`), `ProjectWorkstreamManager` (`project-workstream-manager.js:10`), `ProjectLinksPanel` (`project-links-panel.js:28`, delegated writes in `project-links-manager.js:103`) | **GAP PERF-2** — status/workstream are optimistic, but links have no visible insert/remove receipt. | **OK** for status/workstream; **GAP PERF-2** for link-dialog focus. | **OK** — all use guarded RenderSafe page resolution. | **OK** — bounded frontmatter/link reads. |
| Project | `ProjectDocsIndex` (`project-docs-index.js:33`), `SectionHub` (`section-hub.js:31`), `EpicCreateAction` (`epic-create-action.js:7`) | **GAP PERF-2** — document/section creates wait on persistence and Dataview repaint. | **GAP PERF-2** — new-card focus and insertion position are not restored on rejection. | **OK** — guarded current-page/container reads. | **OK** — one scoped project query per surface. |
| Project | `ProjectDashboard` (`project-dashboard.js:27`), `ProjectWorkstreams` (`project-workstreams.js:10`), `ProjectsHubCards` (`projects-hub-cards.js:10`), `EpicDashboard` (`epic-dashboard.js:8`) | **N/A** — read-only dashboards. | **N/A** | **OK** — RenderSafe or explicit current-page guards. | **GAP PERF-2/PERF-10** — dashboard composition repeats broad page scans (six in `ProjectDashboard`). |
...
| Finance | `BudgetAllocationsEditor` (`platform/blueprints/finance/helpers/budget-allocations-editor.js:23`), `BudgetCategoriesEditor` (`budget-categories-editor.js:20`), `PaycheckExpensesEditor` (`paycheck-expenses-editor.js:18`), `InvoiceControls` (`invoice-controls.js:12`), `InvoiceTimeLogEditor` (`invoice-time-log-editor.js:10`) | **GAP PERF-3** — direct frontmatter helpers repaint after persistence without shared optimistic receipts. | **GAP PERF-3** — budget editors capture scroll, but exact editor focus/selection is not restored. | **OK** — page reads are guarded and render harnesses cover partial input. | **N/A** — active-note frontmatter surfaces. |
...
| Trips | `TripEntryList` (`platform/blueprints/trips/helpers/trip-entry-list.js:31`) | **OK** — checkbox edits use `RenderSafe.mutate` with optimistic/revert callbacks. | **OK** — shared lifecycle captures before write. | **OK** — RenderSafe page guard. | **N/A** — active trip-frontmatter list. |
| Trips | `TripLinks` (`trip-links.js:23`) | **GAP PERF-4** — persistence is routed through `mutate`, but link insertion/removal lacks a structural receipt. | **GAP PERF-4** — dialog/row focus is not restored on failure. | **OK** — RenderSafe page guard. | **N/A** — active trip-frontmatter links. |
