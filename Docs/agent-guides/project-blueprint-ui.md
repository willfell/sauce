...
| `SectionLabel` | Small uppercase muted label + hairline divider above. Replaces every `dv.header(3, ...)` call. Helpers own the label for their section; templates do NOT carry `## H2` headings anymore. | `platform/blueprints/project/helpers/section-label.js` | `ProjectMeetingsPanel`, `ProjectWorkstreamManager`, `ProjectDocsIndex`, `SectionHub`. |
