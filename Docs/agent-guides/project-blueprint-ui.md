...
| --- | --- | --- | --- |
| `ProjectChromeBar` | **The project blueprint's thin adapter over the shared `chrome-bar` mechanism** (chrome-bar extraction). Keeps all project-specific logic — `detectContext` (copied verbatim from `ProjectNavButtons`), the pure `_surfaceSpec(context)` (`{ primary, overflow, leaf }`), `_dispatch(dv, ctx, id)` (routes each action id to its existing helper), `_navEntries` (This-project destinations; the Vault section delegates to `ChromeBar.vaultEntries`), `navTarget`, `_openNavTarget` — and exposes `_adapter()` + a thin `render(dv)` that calls `customJS.ChromeBar.render(dv, this._adapter())`. Supplies the `pcb-root` / `pcb-btn pcb-btn-<variant>` marker classes so the rendered bar stays byte-identical. `ProjectCommandsInit` reuses `_dispatch` / `navTarget` / `_openNavTarget` unchanged. | `platform/blueprints/project/helpers/project-chrome-bar.js` | Every migrated project template (hub / Docs / section hub / doc-note / Map / Board / Task hub / Task note / Links hub / project-todo). |
| `ChromeBar` | **The shared per-surface chrome bar** (`chrome-bar` mechanism; canonical for `Go ▾`/primary/`⋯`-shaped controls). `render(dv, adapter)` owns the whole bar (breadcrumb-left + controls-right + `MenuPopover` wiring + dedupe root); `renderChromeButton(parent, {cls,label,icon,onClick})` is the 32px icon-first hover-lift/press-scale button look; `CHROME_ICONS` = compass/chevronDown/moreHorizontal; `vaultEntries(dv, open)` builds the Go launcher's Vault section (every registry source, ordered by `SpaceNavButtons.firstEntryPerSource`, as a 2-column grid). A blueprint adopts it via an adapter `{ resolve, navEntries, dispatch, openNavTarget, rootClass, btnClass }`. `AccentButton` stays for one-off buttons elsewhere. | `platform/mechanisms/chrome-bar/chrome-bar.js` | `ProjectChromeBar` (first consumer); any blueprint via `depends_on: chrome-bar`. See [`note-chrome.md`](note-chrome.md) §1d. |
