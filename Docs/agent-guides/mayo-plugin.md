...
Sauce's own automation never depends on the installed plugin: `scripts/autoloop/codex-coordinator.js` and `scripts/autoloop/board-health-launchd.js` require the resolver at `scripts/autoloop/loop-config.js` directly, so the coordinator keeps working whether or not the plugin is installed. The plugin's copy at `plugins/mayo/scripts/loop-config.js` serves the skill surface (`${CLAUDE_PLUGIN_ROOT}/scripts/loop-config.js`) and Codex's `codex.plugin_root`. Both now live in this repo; they are still two files and are kept in sync by hand until the collapse lands.
...
Legacy names still answer as deprecation aliases: `/delivery-status` → `/mayo:status`, `/delivery-review` → `/mayo:review`, `$card-intake` → `$mayo-intake`, `$slice-plan` → `$mayo-plan`, `$sauce-autoloop` → `$mayo-run`. The live cron loop still runs `/sauce-autoloop` (fat command, deliberately untouched) until `/mayo:run` is validated — see the design doc's follow-ups.
