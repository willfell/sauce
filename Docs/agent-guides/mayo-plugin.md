---
purpose: Onboarding runbook for the mayo plugin — install/reload for Claude and Codex, the /mayo:init binding walkthrough, and the two round-one test scripts.
load_when: Installing, updating, binding, or debugging the mayo plugin surface; onboarding a new repo/project onto the meta-loop.
---

# Loop plugin — onboarding runbook

The `mayo` plugin centralizes the meta-loop skill surface (status, review, propose, plan, execute, run, intake, init, block-review) in ONE place — `plugins/mayo/` in this repo, published via sauce own marketplace — and every repo binds itself to a vault/board with a committed `.loop/config.json`. A plugin reload puts every session of every project on the same logic. Design: `Docs/superpowers/specs/2026-07-26-meta-loop-plugin-design.md`.

## Architecture in three sentences

Skill bodies live once in `plugins/mayo/skills/*/SKILL.md`; Claude reads them as an installed plugin (`/mayo:*`), and Codex reads the SAME bodies through generated `.agents/skills/mayo-*` routers that resolve the body location from `.loop/config.json` (`codex.plugin_root`) at use time. The binding contract `.loop/config.json` declares vault root, project slug, board/cards paths, id prefix, policy knobs, and coordinator location; the plugin's resolver (`loop-config.js`) turns it into env (`DELIVERY_*`, `SAUCE_LOOP_*`) that retargets the unchanged coordinator, batch-runner, digest, and intake rails at that board. Board-writer authority is untouched: the coordinator remains the only operational writer and card-intake the only planning writer — the plugin skills orchestrate those CLIs and never hand-edit boards. mayo is a consumer of the Sauce engine (`platform/engine/`, see [engine.md](engine.md)): `/mayo:run` can hand a slice to the shipped delivery-slice graph, and `/mayo:status` folds engine runs into its digest.

Sauce's own automation never depends on the installed plugin: `scripts/autoloop/codex-coordinator.js` and `scripts/autoloop/board-health-launchd.js` require the resolver at `scripts/autoloop/loop-config.js` directly, so the coordinator keeps working whether or not the plugin is installed. The plugin surface at `plugins/mayo/scripts/loop-config.js` is a thin forwarder onto that same module, so the skill bodies' `${CLAUDE_PLUGIN_ROOT}/scripts/loop-config.js` contract and Codex's `codex.plugin_root` resolve to one implementation that cannot drift from it.

## Install (Claude, once per machine)

```text
/plugin marketplace add willfell/sauce
/plugin install mayo@sauce
```

Restart the session (or open a new one) — `/mayo:status`, `/mayo:init`, etc. are now available in EVERY repo.
...
then restart the session. Repos can auto-recommend the plugin: `/mayo:init` offers to write `extraKnownMarketplaces` + `enabledPlugins` into the repo's `.claude/settings.json`, which prompts anyone opening that repo to install.

## Install (Codex, per repo)

Codex never reads `~/.claude/`. Its surface is the generated `.agents/skills/mayo-*` routers committed in each bound repo, which point at the canonical bodies via `codex.plugin_root` in `.loop/config.json`:

- Consumer/brew setups: `plugin_root = /opt/homebrew/opt/sauce/libexec/plugins/mayo` — **reload = `brew upgrade sauce`**, nothing else (routers bake no paths and never go stale).
- Dev setups: point `plugin_root` at a local sauce clone`s `plugins/mayo`.

Regenerate routers only when the SKILL SET changes: `node <plugin_root>/scripts/gen-codex-routers.js --repo . --json` (also run by `/mayo:init`). CI keeps the workshop's own routers honest via the `--check` byte-determinism gate.

## Bind a repo: /mayo:init walkthrough

From the repo you want on the loop:

1. `/mayo:init` — answer the interview: vault root, project slug, board paths (convention offered: `spice/projects/<slug>/<slug>-board.md` + `tasks/`), default id prefix, policy (batch policy, execution mode, deploy subscriptions/vaults), coordinator (`brew` default), optional FID path, Codex routers y/n.
...
3. Commit `.loop/config.json` + `.agents/skills/mayo-*` to the repo.
4. `/mayo:status` — first read of a fresh binding legitimately shows an empty ledger.
...
- **`policy.run_scope`** (default `"board"`) — how far a live `/mayo:run` drives the board before stopping: `"board"` keeps taking eligible work in board order, epic by epic, until the frontier drains, a ceiling/halt applies, or every eligible card is leased by another session (`all-work-leased`); `"epic"` completes one epic then stops; `"turn"` is one bounded claim-or-resume turn. With the default, the start prompt is IDENTICAL for every bound repo: `Use $mayo-run --live. Start NOW — do not stop after acknowledging.` — scope, deploy posture, and gates all come from the binding and the skill bodies, never the prompt.
...
| Slash | Codex | Does |
| --- | --- | --- |
| `/mayo:init` | `$mayo-init` | bind repo → vault/board; write config; generate routers |
| `/mayo:status` | `$mayo-status` | phone-sized read-only digest + since-you-last-looked |
| `/mayo:review` | `$mayo-review` | walk the retroactive digest; decide escalations |
| `/mayo:propose` | `$mayo-propose` | design dialogue → epic proposal doc |
| `/mayo:plan` | `$mayo-plan` | plan AS board schema; prompts id prefix + priority; mints via intake |
| `/mayo:execute` | `$mayo-execute` | drive the minted epic in-session, sub-agent per slice, full quorum |
| `/mayo:run` | `$mayo-run` | one bounded autonomous loop turn (the run-loose engine) |
| `/mayo:intake` | `$mayo-intake` | raw requirement → board-ready work (incl. supersede-at-mint) |
| `/mayo:block-review` | `$mayo-block-review` | detect + heal dangling depends_on rot; auto-fix provable, escalate never-minted |

The legacy aliases (`/delivery-status`, `/delivery-review`, `$card-intake`, `$slice-plan`, `$sauce-autoloop`) were retired in the engine-reintegration cycle; use the `/mayo:*` / `$mayo-*` names above. The fat `/sauce-autoloop` command and its `sauce-autoloop.plist.sample` are retired with them: unattended cadence is now `sauce run <note> --install-launchd` (see `platform/engine/sauce-engine.plist.sample`), and `/mayo:run` has an opt-in engine mode that runs `platform/engine/graphs/delivery-slice.md` — the prose pipeline stays the default until one live epic completes through the graph and its ledger is diffed against a coordinator run. See [engine.md](engine.md).

## Test script A — sauce/headspace (full cycle with a toy epic)

Run in a fresh Claude session in the sauce workshop repo:

1. `/plugin marketplace add willfell/sauce` → `/plugin install mayo@sauce` → restart session. (Workshop is already bound — `.loop/config.json` is committed.)
2. `/mayo:status` — expect the real board digest, byte-for-byte the same content `/delivery-status` used to give.
3. `/mayo:propose` a TOY epic — e.g. "a `docs/toys/loop-plugin-smoke.md` note describing the plugin smoke test". Expect: one-question-at-a-time dialogue, proposal written under `spice/projects/sauce/docs/proposals/`.
4. `/mayo:plan` against the proposal — expect prompts for **id prefix** (answer `TOY`) and **board priority position** (answer `bottom`), a dry-run receipt to approve, then mint via the intake rail, then the "execute now or leave for the loop?" question — answer **leave for the loop**.
...
6. Clean up so the live board stays clean — discard through the coordinator (the only sanctioned deleter):
   `node <coordinator> discard --card "<toy epic slice/epic name>" --reason "plugin onboarding smoke test" --json` per the coordinator's discard usage (dry-run first if unsure), then `/mayo:status` to confirm the tombstone shows in "since you last looked".
7. Repeat step 2 from Codex (`$mayo-status` in the sauce repo) — same digest, same body, different runtime.
...
1. `/mayo:init` — vault root `~/obsidian/ero-sauce`, slug `ero-egnyte-mcp`, board `spice/projects/ero-egnyte-mcp/ero-egnyte-mcp-board.md`, cards `spice/projects/ero-egnyte-mcp/tasks`, prefix (e.g. `EM`), **`observe_only: true`**, deploy subscriptions empty (ero completes on merge), coordinator `brew`, routers yes.
2. `/mayo:status` — expect a digest of the ero board with an empty/near-empty ledger (ero's Python loop owns its state; the coordinator has no claims here). That's the correct round-one answer.
3. Confirm `/mayo:plan` REFUSES (observe_only) — the guard that protects ero's existing loop.
4. Commit `.loop/config.json` + routers to egnyte-mcp when satisfied.

## Troubleshooting

- `config_missing` → run `/mayo:init`. Any other refusal code from `loop-config.js` names its exact fix.
