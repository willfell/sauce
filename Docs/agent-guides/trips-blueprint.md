---
purpose: Canonical reference for the `trips` blueprint — a per-trip folder-is-truth ecosystem under spice/trips/. Note types, collision-free naming, TripSectionKinds, the three render helpers, chrome (Breadcrumb ancestors-mode trail + SectionLabel + launcher nav), and the conformance heal. Read before any trips work.
load_when: Touching the trips blueprint (spice/trips/), its helpers, TripSectionKinds, TripNavButtons, TripsHubCards, TripSectionsCards, the conformance heal, or debugging trips render/chrome/naming behavior.
---

# Trips blueprint

A **per-trip folder-is-truth ecosystem** at `spice/trips/`. Each trip lives in its own slug-named folder containing an atlas note (the trip homepage) plus section notes (Flights, Stay, Packing List, To Do, Notes) and an optional kanban board sub-folder. The hub at `spice/trips/Trips.md` lists all trips as BeaconCards grouped by temporal status. `module_directory: trips`. Headspace-only subscription (the only consumer vault subscribed to trips).

## The one-paragraph model

Three note types — `trips-hub` (the single root `spice/trips/Trips.md`), `trip` (one atlas per trip, the folder homepage), `trip-section` (zero-or-more per trip: Flights, Stay, Packing List, To Do, Notes, custom). **Folder-is-truth: the slug folder IS the trip.** Frontmatter carries identity (`type`, `section_kind`, `section`, `trip`, `trip_slug`) — never structural position — so sections auto-discover each other via the folder, moves are just file renames, and adding a custom section gets automatic nav coverage without any registry update. Every note renders the same chrome stack: `Breadcrumb` → `SpaceNavButtons` → `TripNavButtons` → content.
...
The `_createTrip` flow in `TripNavButtons` enforces this: the atlas is written to `${tripDir}/${atlasBase}.md`, and each section to `${tripDir}/${atlasBase} — ${label}.md`. Custom sections created via "+ New Section" follow the same pattern: `${tripDir}/${atlasBase} — ${title}.md`.

## `TripSectionKinds` — the section single source of truth

`platform/blueprints/trips/helpers/trip-section-kinds.js` (runtime: `customJS.TripSectionKinds`) is the **SSOT for the 5 default section kinds**. Both `TripNavButtons` and `TripSectionsCards` consume it — this prevents the label/icon/order drift that existed before the refactor when the two helpers maintained independent lists.
...
The per-trip nav block. Renders differently by context (detected from `file.path`):

- **On the hub (`trips-hub`):** renders a `+ New Trip` accent button that opens a creation dialog (name + slug preview + start date + end date + location → `_createTrip` scaffolds the slug folder, atlas, all 5 default sections from templates, and a board subfolder).
- **On atlas, section, board, or board-card notes:** renders a `Trip` SectionLabel band (hairline + muted uppercase label) + a **full-width PRIMARY button** back to the atlas (absent when already on the atlas) + a **"Go to…" launcher pill**.

The **Go-to launcher** is a `document.body` overlay (ported from `SpaceNavButtons`) — a **full-width bottom sheet on mobile** (with drag handle, 72 vh, safe-area inset padding) and an **anchored dropdown on desktop** (300 px wide, positioned below the pill). It lists: other sections ordered by `TripSectionKinds.order(section_kind)` then alphabetically within the same order, then the Trip Board (if it exists), then a `+ New Section` action at the bottom. Selecting `+ New Section` prompts for a title and creates `<atlasBase> — <title>.md` with canonical `trip-section` frontmatter and the full chrome stack. A single `close()` teardown handles backdrop-tap, Escape, and re-tap toggle so no keydown listener leaks.

Context detection reads `file.path` parts: `spice/trips/Trips.md` → hub; `spice/trips/<slug>/<name>.md` with `type: trip` → atlas; same path but different type → section; `spice/trips/<slug>/board/<slug>-board.md` → board; `spice/trips/<slug>/board/<card>.md` → card.
...
Every trip note (hub, atlas, section) renders the same chrome header:

```
Breadcrumb  →  SpaceNavButtons  →  TripNavButtons  →  content
```

Each is a separate `dataviewjs` block calling `customjs-guard`.

**Breadcrumb** uses the shared `breadcrumb` mechanism's **`ancestors` mode** (not `path_walk`). The manifest declares per-type breadcrumb atoms:
...
**Board exemption:** the kanban board at `spice/trips/<slug>/board/<slug>-board.md` (type `kanban`) keeps its `## Column` headings (kanban columns must be H2 in Obsidian's kanban plugin). The board is **outside the breadcrumb scope** — its `type: kanban` would collide with the project board type and it is not in the breadcrumb registry. `TripNavButtons` handles nav on the board via the same Trip band + primary + Go-to launcher, so the board still has full nav coverage.

## Install heal — `applyTripsConformanceHeal` (in `platform/install.js`)

An **ungated backfill** (runs every install; not version-gated per the migration-lifecycle rule, because it renames real user notes). Migrates pre-refactor trips that used generic folder-wide basenames (`Trip Atlas.md`, `Trip Flights.md`, etc.) to the collision-free canonical shape.
...
4. Applies per-note transforms: `_tripMigrateCreatedAt` (legacy `created:` → `created_at:`, coerces date-only to ISO+TZ), `_tripStripLoneTripTag` (removes lone `tags: [trip]` block), `_tripSetFmKey` (replace-or-insert canonical `type`/`section_kind`/`section`/`trip`/`trip_slug` fields), `_tripInjectBreadcrumb` (idempotent — checks for `class: "Breadcrumb"` substring first), `_tripRepairAtlasLinks` (`[[Trip Atlas]]` → `[[<atlasBase>]]`).
5. Hub: injects Breadcrumb, converts `## All Trips` → SectionLabel. Atlas: injects Breadcrumb, converts `## Mentions` → SectionLabel (only when a BacklinkPanel block is also present).
...
**Idempotency contract:** a second run writes ZERO files. All change-detection is existence/substring/regex-based against the already-canonical state: canonical filenames already match, `class: "Breadcrumb"` present, `section_kind` present, `## All Trips` / `## Mentions` headings already converted → all transforms are no-ops.
...
`customjs_classes`: TripSectionKinds, TripsHubCards, TripNavButtons, TripSectionsCards. `depends_on`: nav-buttons, customjs-guard, cards, convenience, backlink-panel, render-safe. `nav_buttons[]`: the Trips button (`openLink → spice/trips/Trips.md`, order 110). `breadcrumb.types[]`: trips-hub + trip + trip-section (ancestors mode).
...
- **`TripSectionKinds` is the SSOT.** Never hardcode section labels or icons in `TripNavButtons` or `TripSectionsCards` — always go through `customJS.TripSectionKinds`. The Node-side copy in install.js must be kept in lockstep.
- **`section_kind: custom`** is the catch-all for user-created extra sections (e.g. "Honorees", "Restaurants"). `TripSectionKinds.order("custom")` returns 999, so custom sections always sort after the 5 defaults.
...
- **Board is out of breadcrumb scope.** Its `type: kanban` clashes with the project kanban type; do not add it to the breadcrumb registry. TripNavButtons gives it nav coverage instead.
