...
- **The primitive:** `customJS.SectionLabel.divider(containerEl)` renders the canonical hairline — `border-top: 1px solid var(--background-modifier-border); margin: 12px 0` — the single source of the "happy medium" spacing (wiki-aligned 2026-07-02: was `8px 0`; matched to the wiki helper-owned dividers). Tune the gap in that one method. Guard it: `if (customJS?.SectionLabel?.divider) customJS.SectionLabel.divider(el);`.
