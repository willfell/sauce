...
- **`WikiLeafActions`** — the action row on wiki-page leaves. Renders a top `<hr>`, then **ONE horizontal row** — `[Wiki] [<section-name>]` on the left, `[Move]` pushed to the far right (`margin-left:auto`, no wrap) — then a **bottom `<hr>`**. The leaf owns BOTH dividers, so the page template carries no trailing `---`. Buttons are sized by `_styleLeafBtn` (readable, natural width; `{shrink}` lets a long section label ellipsis so the row never overflows on a phone; `{right}` pushes Move over). Move-options are computed **lazily on click** (never at render) so a cold-loading `WikiMove` can't throw and blank the row. The Move dialog is an **indented tree** (see below).
