...
- **Dividers** are owned by the action helpers (`<hr>` with tight `2px` margins), NOT the templates — so there are no extra line gaps between the nav bar → divider → wiki buttons → search/content. Do not re-add `---` to the templates. On hub/section notes this only works because `WikiTree` renders `WikiHubActions` inline: two adjacent dataviewjs blocks always get an Obsidian inter-block gap no `<hr>` can close, so the ONLY way to make a divider tight against BOTH the buttons above and the search below is to render them in the same block.
