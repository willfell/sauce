...
- **DO make the PR TITLE carry the highest commit type on the branch.** PRs land by **squash**-merge, so the branch collapses into one commit whose subject is the **PR title** — that title is the only conventional commit the bumper ever sees. Branch-local commit types are discarded at merge. A branch with `feat(coordinator):` + `fix(intake):` behind a PR titled `fix(loop): …` releases as a **patch**, not a minor, and `npm run release:plan` will not warn you: run locally it reads the branch's individual commits and reports the *pre-squash* answer. Check the title before merging; the version is not correctable afterward (never hand-tag or re-version to fix it). Incident: v0.281.1 — a release adding a new CLI verb shipped as a patch for exactly this reason.
...
- **TDD against the real path, not just adjacent paths.** Distinguish what a test EXERCISES from what it COULD exercise. v0.93.3 lesson: `runBootstrap` end-to-end with `skipInstaller: true` exercises `phaseFetchPlugins` but NOT `applyExternalPlugins`; a true regression net for the install-time path requires invoking `platform/install.js` end-to-end on a synthetic existing vault. **v0.284.3 repeat of the same lesson, one level up:** a heal whose tests all called the heal function directly passed green while the heal was *never invoked at all* during a real install (landmine #32) — calling a heal directly proves it WORKS, never that it RUNS. For anything install-gated, drive `install()` itself; a vault subscribed to nothing is the strongest form, since it proves the behavior isn't a side effect of some component's version-gated `installItem` pass.
