...
| **v0.99.1+** | `cowork:doctor` (embedded in the nightly reconciler; 4 checks locked at the 2026-06-11 brainstorm) + cross-machine lockfiles/path-relative wrappers (v0.99.2+, bundled) + `cowork:audit-cohesion` + per-skill SemVer + the v0.95.1-migrator carry-forward | queued | Quality observability + Cross-machine consistency + Cohesion enforcement |
