/**
 * Planning Board Projects (CustomJS)
 * Renders the active projects list on the Planning Board as activity-sorted
 ...
 * kanban board file.
 *
 * Source query: pages under boards/planning/ tagged #project, excluding
 * Planning-Board itself, anything in /steps/, and *-board files.
 *
 * Usage in DataviewJS:
 *   await dv.view("Extras/Scripts/customjs-guard", { class: "PlanningBoardProjects" });
 */
class PlanningBoardProjects {
    async render(dv) {
        // Get project hub files
        const projectHubs = dv.pages('"boards/planning"')
            .where(p => p.file.etags.includes("#project") && p.file.name !== "Planning-Board" && !p.file.path.includes("/steps/") && !p.file.name.toLowerCase().endsWith("-board"));
