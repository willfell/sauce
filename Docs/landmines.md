...
### 12. `.obsidian/plugin-data` ban-lift allowlist + `.claude/skills/` skill-materialization carve-out + `.claude/commands/` slash-command carve-out — fifteen sanctioned write zones, only via the installer/bootstrap, only with the four safety mechanics

The `.obsidian/` ask-before-acting gate is lifted for exactly thirteen paths, PLUS one peer top-level zone `.claude/skills/<subtree>` (added in v0.30.0; see end of this entry):
