...
- **Initial baseline:** 0 violations + 10 untracked top-level dirs + 9 rules-file missing warnings (captured 2026-05-08 22:17 UTC; opening report `ranch/audits/2026-05-08-221708-audit-opening.md`)
- **Current state:** 0 violations + 7 untracked dirs (after session 1; closing report `ranch/audits/2026-05-08-222543-audit-closing.md`)
- **Sessions:** 1 (see "Session log" below)
- **Outstanding pre-rollup blockers:**
  - **Rules-file warnings (9 / 9 blueprints).** Same gap as accuris + headspace — vault hasn't pulled v0.29.0 platform updates yet. Run `sauce update` against ero-sauce before next cleanup session so violations populate.
  - **Workshop wikilink-rewrite gap surfaced this session (v0.29.1 carry candidate).** `LEGACY_PATH_SUBSTITUTIONS` lacks a rule for `Projects/<TitleCase>/` ero-shape source paths. Pre-migration ero stored projects at root-level `Projects/<TitleCase>/<TitleCase>.md` (user-named-with-spaces; not in `boards/planning/` like accuris/headspace). The migrator moved them to `spice/projects/<TitleCase>/` preserving the casing, but `[[Projects/<TitleCase>/<TitleCase>]]` wikilinks in remaining files weren't rewritten. Hand-fixed in `spice/boards/Projects.md` + `spice/boards/Project Work.md` this session; future ero re-migrations would still need manual cleanup until the rule is added.
  - **Title Case spice/projects/ slug dirs coexist with kebab-case.** Two separate pre-migration source paths produced two slug-format conventions: legacy `Projects/NEPA Rag/` → `spice/projects/NEPA Rag/` (Title Case preserved); legacy `boards/planning/nepa-rag/` → `spice/projects/nepa-rag/` (kebab). Result: `NEPA Rag/` (1 .md, kanban-shape with planning items "[[Determine Sources of Information what should be Sourced]]" + LlamaIndex link + "[[Create Initial POC]]") AND `nepa-rag/` (3 .md: canonical atlas + board + task note `Create Initial Design.md`) coexist as separate dirs. Same pattern for `Request for Proposal Trigger Flow/` (1 .md, kanban-shape, NO kebab equivalent in this vault). User-judgment item: are these dupes (delete) or unique-content-not-yet-merged (preserve + future merge)?
- **Deferred / accepted residue (pending v0.29.1 audit-allowlist):** `.smart-env/` (Smart Connections plugin index, plugin-managed at vault root by design — same posture as accuris/headspace), `Cowork/` (16 .md AI workflow user content — same posture), `Docs/` (46 .md user docs across plans/ + Meta/ + Audits/ + Scripts/; namespace-clash with workshop `Docs/` — same posture), `Extras/` (31 .md remaining: 25 Templates/ + 6 Snippets including 5 themes — pre-migration platform shape, user customizations potential), `Files/` (Attachments/ subdir with 7 PDFs/.docx/.jpg — user assets), `MOCs/` (6 user Maps-of-Content + Meta/Scripts subdir; same posture as accuris)
- **Deferred for user judgment:**
  - **`Finance/`** (14 .md substantive finance content: Finance-Hub, Finance-README, monthly subdirs 2026-01 through 2026-05 with Invoice + board + Time-Log; plus 03-2026-Timesheet.pdf in 2026-03/) — parallel-living with `spice/finance/` install-time stubs. Same posture as headspace's Finance/. Pending finance-migrator (v0.30.0 carry per CLAUDE.md) OR explicit user direction.
  - **Root-level user files (NOT counted in audit's untracked-dir count):** `+Home.md` (299-byte user home note), `CLAUDE.md` (17KB user vault claude config), `2026-04-Timesheet.pdf`, `2026-05-04 Egnyte Connector - One Month In.pptx`, `Egnyte Connector - Prompting Guide (DRAFT).docx`, three loose images (`image 1.jpg` 3.0MB, `image.jpg` 3.7MB, `Pasted image 20260501091133.png` 145KB). User-meaningful content of unclear schema-fit; sanctioned new top-level dirs only allow `spice/`/`pantry/`/`ranch/`/`assets/`/`.obsidian/`/`.claude/` so root-level user notes need user direction.
  - **`ero-*.skill` files at vault root** (5 files: ero-invoice/-meeting/-session/-status/-task.skill — Claude Code skill definitions). Likely belong in `.claude/skills/` (sanctioned) but moving requires user-confirmed Claude Code config behavior.
  - **`spice/boards/Project Work.md`** (kanban with 3 user task items + fixed `[[spice/projects/nepa-rag/Project|nepa rag]]` wikilink). User-named planning kanban; doesn't fit any sauce blueprint shape. Either accept as personal kanban OR delete if obsolete.
  - **Title Case `spice/projects/<TitleCase>/` dirs** — see "Outstanding pre-rollup blockers" above. `NEPA Rag/` may have unique planning content not in `nepa-rag/`'s board file; `Request for Proposal Trigger Flow/` has no kebab equivalent at all (orphan or renamed?).
...
### 2026-05-08 — ero-sauce session 1

- **Snapshot:** `ero-sauce.pre-cleanup-20260508-221649/` (sibling of audited vault; 56M, full tree copy)
- **Before:** 0 violations, 10 untracked dirs
- **After:** 0 violations, 7 untracked dirs (-3)
- **Audit reports:** opening `ranch/audits/2026-05-08-221708-audit-opening.md` → closing `ranch/audits/2026-05-08-222543-audit-closing.md`
- **Fixed:**
  - **Phase 1 — root-level residue cleanup.**
    - Moved `claude-sync/commands/{audit,project}.md` → `.claude/commands/` (sanctioned per CLAUDE.md); rmdir empty `claude-sync/`.
    - Deleted root `migration-plan.json` + `migration.log` (v0.28.0 migration artifacts, single-use).
    - Deleted root `~$2026-05-04 Egnyte Connector - One Month In.pptx` (Office lock file — application crash artifact, not user content).
    - Deleted root `Untitled.base` (0-byte empty Bases stub) + `2026-04-26-Briefing.md` (0-byte empty stub).
    - Deleted `Projects/temp.md` (1-byte residue) + rmdir empty `Projects/` (legacy source had `Projects/temp.md` + 2 project subdirs `NEPA Rag/` + `Request for Proposal Trigger Flow/`; the project subdirs migrated to `spice/projects/<TitleCase>/`, leaving only `temp.md` as orphan).
  - **Phase 2 — project schema migration to project@1.3.8 (workshop ships canonical via `b22c390`).** Wrote `/tmp/ero-project-schema-migration.js` (deleted after run) — same shape as headspace Phase 9: rename atlas + add canonical frontmatter fields + vault-wide wikilink rewrite via `require()`-ing workshop `platform/migrate/wikilink-rewrite.js` `rewriteString()`. **7 kebab-case projects renamed** (atlas `<DisplayName>.md` → `Project.md`):
    - `egnyte-copilot-connector/Egnyte Copilot Connector.md` → `Project.md` (+ `name: "Egnyte Copilot Connector"`)
    - `egnyte-mcp/Egnyte - MCP.md` → `Project.md` (+ `name: "Egnyte - MCP"`)
    - `microsoft-egnyte-connector-rollout/Microsoft - Egnyte Connector Rollout.md` → `Project.md` (+ `name: "Microsoft - Egnyte Connector Rollout"`)
    - `nepa-rag/NEPA Rag.md` → `Project.md` (+ `name: "NEPA Rag"`)
    - `resolve-egnyte-connector-user-permissions/Resolve Egnyte Connector User Permissions.md` → `Project.md` (+ `name: "Resolve Egnyte Connector User Permissions"`)
    - `rfp-folder-scaffolding/RFP-Folder-Scaffolding.md` → `Project.md` (+ `name: "RFP Folder Scaffolding"`)
    - `unanet/Unanet.md` → `Project.md` (+ `name: "Unanet"`)
    - All 7 atlases also got `type: project` + `description: ""` (preserved if already populated — `unanet` + `rfp-folder-scaffolding` had real descriptions kept) + `workstreams: []` per rule_fragment requirements.
    - **Vault-wide wikilink rewrite:** 32 files touched, 68 matches replaced — `[[<DisplayName>]]` / `[[<DisplayName>|alias]]` → `[[spice/projects/<slug>/Project|<original-display>]]` to disambiguate the now-shared `Project.md` filename.
    - **LEGACY_PATH_SUBSTITUTIONS pass:** 10 additional files touched (cleaning up any straggler legacy paths the v0.28.0 migrator had missed pre the v0.29.1-carry table extensions).
  - **Phase 3 — in-vault `ranch/` byte-equivalence patches** (mirrors headspace Phase 9 pattern so `sauce update` is no-op-effectively for these files when ero pulls workshop project@1.3.8):
    - `ranch/scripts/project/projects-hub-cards.js:54` — `title: (p) => p.file.name` → `(p) => p.name || p.file.name`.
    - `ranch/scripts/project/project-nav-buttons.js:340-342` — New Project writeTpl: atlas `${name}.md` → `Project.md`; map `${name} - Map.md` → `Project Map.md`. Board unchanged at `${slug}-board.md`.
    - `ranch/templates/Template, Project.md` — added `type: project` + `name: "{{NAME}}"` lines after the `---` opener.
    - Verified byte-equivalence via `diff` against workshop canonical: `projects-hub-cards.js` BYTE-EQUIVALENT; `project-nav-buttons.js` differs only in `{{templates_path}}` → `ranch/templates` substitutions (expected, applied at install time); `Template, Project.md` BYTE-EQUIVALENT.
  - **Phase 4 — `spice/boards/` non-canonical cleanup.**
    - Deleted `spice/boards/Projects-Hub.md` (pre-migration platform-shape projects-hub with broken `dv.pages('"Boards/planning" AND #project')` query + duplicate of canonical `spice/projects/Projects.md` which already exists with the post-v0.28.0 sauce shape).
    - Fixed `spice/boards/Projects.md` (user planning kanban — KEPT, not deleted): the migration script auto-rewrote `[[Projects/NEPA Rag/NEPA Rag|NEPA Rag]]` → `[[spice/projects/nepa-rag/Project|NEPA Rag]]`. The remaining stale wikilink `[[Projects/Request for Proposal Trigger Flow/Request for Proposal Trigger Flow|...]]` couldn't be auto-rewritten (no kebab equivalent in vault), so hand-rewrote it to point at the still-existing Title Case dir `[[spice/projects/Request for Proposal Trigger Flow/Request for Proposal Trigger Flow|...]]` so the link resolves until user decides on Title Case dir disposition.
    - Hand-rewrote `[[nepa rag]]` (lowercase + space) wikilink in `spice/boards/Project Work.md` to `[[spice/projects/nepa-rag/Project|nepa rag]]` (the migration script's regex was case-sensitive and matched only `NEPA Rag`).
  - **Phase 5 — Timestamps/ via script.** Wrote `/tmp/ero-timestamps-cleanup.js` (deleted after run) — extends accuris/headspace pattern for ero's specific tree shape (Briefing/EOD pair-files instead of headspace's Journal/ToDo). 16 files triaged: **12 moved, 4 deleted**:
    - **8 daily summaries (briefings + EODs)** → `spice/daily/summaries/<date>/<basename>.md` (4 dates × 2 files each: 2026-04-13/-14/-15/-16; each has both -Briefing.md + -EOD.md)
    - **3 weekly summaries** (Summaries/Weekly/) → `spice/daily/summaries/Weekly/<basename>.md`
    - **1 meeting** (Meetings/2025-06-16.md, type:meeting, mostly-empty stub from June 2025) → `spice/meetings/notes/2025/06-June/2025-06-16.md`
    - **3 audit reports deleted** (Audits/2026-04-16-audit.md + audit-2.md + 2026-04-17-audit.md — historical user-vault audit reports, regenerable)
    - **1 customjs-test deleted** (2026/05-May/2026-05-02-customjs-guard-test.md — one-off test doc, no blueprint shape)
    - All moves applied LEGACY_PATH_SUBSTITUTIONS via `rewriteString()` to fix any pre-migration path refs.
    - rm -rf `Timestamps/` (whole tree).
- **Remains (untracked dirs after session):**
  - `.smart-env/` (Smart Connections plugin index — same posture as accuris/headspace)
  - `Cowork/` (16 .md AI workflow user content — context/, prompts/, prompts/archive/ subdirs; same posture as accuris/headspace)
  - `Docs/` (46 .md user docs: plans/ + Meta/ + Meta/Audits/ + Meta/Scripts/ subdirs; namespace-clash with workshop's `Docs/` — same posture as accuris/headspace)
  - `Extras/` (31 .md: 25 Templates/ + 6 Snippets/ including 5 Baseline Themes/ + 1 Baseline Snippets.md — pre-migration platform shape, potential user customizations vs `ranch/templates/`)
  - `Files/` (7 attachments in Files/Attachments/: 4 PDFs + 2 docx + 1 jpg)
  - `Finance/` (14 .md substantive finance content: Finance-Hub + Finance-README + 2026-01 through 2026-05 monthly subdirs each with Invoice/board/Time-Log + 03-2026-Timesheet.pdf in 2026-03/ — deferred per Outstanding pre-rollup blockers above)
  - `MOCs/` (6 user MOC files + Meta/Scripts/ subdir with 2 .sh scripts)
- **Notes:**
  - **NEW workshop carry candidate (v0.29.1 OR v0.30.0): `LEGACY_PATH_SUBSTITUTIONS` rule for `Projects/<TitleCase>/` → `spice/projects/<???>/`**. Pre-migration ero stored projects at root-level `Projects/<Title Case Slug>/<Title Case Slug>.md` (NOT `boards/planning/<kebab>/` like accuris/headspace). The migrator preserved the legacy Title Case dir name when moving from `Projects/<TitleCase>/` so the resulting path is `spice/projects/<Title Case Slug>/...`. Wikilinks pointing at the legacy `Projects/<TitleCase>/` paths weren't rewritten because no rule existed. Hand-fixed in `spice/boards/Projects.md` + `spice/boards/Project Work.md` this session. Same shape class as v0.29.1 wikilink-rewrite carries (similar to headspace's `boards/side-quests/` table gap). The rewrite is non-trivial because the migrator preserved Title Case (not slugified), so a one-to-one path rewrite is correct: `Projects/X/Y` → `spice/projects/X/Y`.
  - **NEW workshop carry candidate (v0.30.0 design discussion): Title Case vs kebab-case slug coexistence in `spice/projects/`**. Two pre-migration source paths produce two slug-format conventions: legacy `Projects/<TitleCase>/` migrates to Title Case-preserved `spice/projects/<TitleCase>/`; legacy `boards/planning/<kebab>/` migrates to kebab-preserved `spice/projects/<kebab>/`. In ero this resulted in `NEPA Rag/` AND `nepa-rag/` coexisting (same project, different sources merged separately). The migrator should either (a) slugify legacy Title Case dir names to kebab-case during migration (consistent with landmine #19 lowercase-platform-managed-dirs invariant) OR (b) detect duplicate-by-content and merge during migration. Either is a substantive design discussion; deferred to v0.30.0 cycle.
  - **Reused workshop `rewriteString()` again** (Phase 2 + Phase 5). Reinforces the pattern from accuris + headspace sessions: `require()` the migrator's wikilink-rewrite.js from one-off vault-cleanup scripts is durable + composable. Three data points across three vaults — strong signal toward productizing as a `sauce migrate-orphans <vault>` verb in a future cycle.
  - **No trips schema migration needed** (handoff suggested it might be) — ero's `spice/trips/` has only the `Trips.md` hub, no per-trip dirs. Hub already byte-identical to canonical workshop `platform/blueprints/trips/content/Trips.md`.
  - **No to-do source_board frontmatter rewrite needed** (handoff suggested checking per headspace Phase 10 lesson) — verified via `grep -h "^source_board:" -r spice/ | sort -u`: only canonical `spice/`-prefixed values present (`spice/boards/To-Do-Board.md` + `spice/projects/<slug>/<slug>-board.md`). Either ero was migrated under a later `LEGACY_PATH_SUBSTITUTIONS` table OR the source-board frontmatter rewrite issue is headspace-specific (its source `boards/To-Do.md` filename differed from sauce's canonical `To-Do-Board.md`, ero apparently aligned at migration time).
  - **`Projects.md` hub at canonical location** (`spice/projects/Projects.md`) is already byte-identical to canonical workshop content. Same for `spice/trips/Trips.md`.
  - **Ero went 10 → 7 untracked dirs** (vs accuris 16 → 11; headspace 7 → 4). Ero's remaining 7 mostly cleanly partition into "explicitly accepted user-owned residue" (Cowork, Docs, Files, MOCs, .smart-env) + "user-judgment substantive" (Finance) + "future audit-allowlist mechanism candidate" (Extras pre-migration shape). Plus root-level user files + `ero-*.skill` files surfaced as user-judgment items not counted in the audit's untracked-dir metric.
  - **Three vaults at session 1 close:** accuris (16 → 11, snapshot retained) | headspace (7 → 4, snapshot retained, trips + projects fully schema-conformant, To-Do-Board consolidated) | ero (10 → 7, snapshot retained, projects fully schema-conformant). All three vaults' projects + trips now match workshop canonical schema; remaining work is per-vault user-judgment on residue dirs + finance-migrator (v0.30.0) + audit-allowlist mechanism (v0.29.1).
  - **Snapshot retained at** `ero-sauce.pre-cleanup-20260508-221649/`. Cleanup intentionally conservative: only deleted clearly-residue items with parity in legacy source vault (`/Users/willfellhoelter/notes/ero-sync/ero`, read-only per landmine #20) AND/OR snapshot. User-owned content + ambiguous-mtime content deferred. Title Case `spice/projects/<TitleCase>/` dirs preserved pending user judgment.
