...
6. If enabled, **Prompt body?** Read current body of `spice/cowork/prompts/<orch>.md` (strip frontmatter; capture trim length). Ask: "Prompt body for cowork:<orch>? Options: (a) use the platform-shipped default for engagement-type <engagement.type>, (b) leave empty stub (orchestrator emits warning: empty_prompt), (c) tell me what this should emit (interactive Q&A — describe desired content)."
   ...
   - **(c)** Interactive Q&A. Ask: "What should cowork:<orch> emit when it fires? Describe the content + structure you want." User responds. **Contract-guard the response BEFORE drafting:** scan the user's natural-language description for any of the case-insensitive substrings `["callout", "patch", "daily note", "Timestamps/", "append to", "into today", "into the daily", "summaries/"]`. If any match, the user is describing the legacy v0.45.0 callout-patching surface (retired in v0.65.0). DO NOT draft a prompt body matching their description verbatim. Instead, reply: "Heads up — that description sounds like the legacy v0.45.0 callout-patching pattern (writing into the daily note). v0.65.0 retired that: each scheduled job now writes ONE atomic note at `spice/cowork/<timeframe>/.../<slug>.md` instead. The prompt body should describe what content goes IN that atomic note (sections, headings, bullets) — not where the content lands. Want me to draft a body that describes the same content but for the atomic-note contract? (yes / no — re-describe)". On `yes`, draft a body that captures the user's intended CONTENT (sections + structure they mentioned) but strips all path / callout / daily-note references. On `no — re-describe`, re-ask the original "What should cowork:<orch> emit?" question. Once the description is contract-clean, draft a prompt body matching it. Show drafted body to user. Ask: "Looks good? (yes / edit / restart)". On yes, write the drafted body to `spice/cowork/prompts/<orch>.md`. On edit, accept their revision and re-run the contract-guard scan before writing. On restart, re-ask step 6.
