...
Table column headers — use these literally so renderers and downstream readers stay consistent. For the finance snapshot, the accounts + transactions tables are the defaults; the other shapes are listed for reference if the consumer extension surfaces them:
...
Skip a section entirely when its gather output is empty. If a section's data exists but is too small for a table (e.g., 1–2 transactions), use bullets inside the admonition.
...
Also append the reason to the atomic note's `warnings:` frontmatter array (e.g., `warnings: [finance_unavailable]`).
